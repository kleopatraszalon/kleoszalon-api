...
export async function ensureFinanceV5Menu(){
  ...
  await safe(`INSERT INTO role_menu_permissions(role_key,menu_id,can_view,can_create,can_edit,can_delete,can_approve,can_export,can_view_financial,can_manage_permissions,scope_type,updated_at)
    ...
    WHERE m.code='finance' OR m.code IN('finance.dashboard','finance.checkout','finance.transactions','finance.cash','finance.partners','finance.payment_categories','finance.documents','finance.online','finance.reports','finance.payment_methods','finance.settings','finance.nav_online_invoice')
  ...
  await safe(`INSERT INTO role_menu_permissions(role_key,menu_id,can_view,can_create,can_edit,can_delete,can_approve,can_export,can_view_financial,can_manage_permissions,scope_type,updated_at)
    SELECT r.role_key,m.id,
      CASE WHEN m.code IN('finance','finance.dashboard','finance.checkout','finance.transactions','finance.cash','finance.partners','finance.documents','finance.online','finance.reports') THEN true ELSE false END,
      CASE WHEN m.code IN('finance.checkout','finance.transactions','finance.partners','finance.documents') THEN true ELSE false END,
      CASE WHEN m.code IN('finance.checkout','finance.transactions','finance.partners','finance.documents') THEN true ELSE false END,
      false,false,
      CASE WHEN m.code IN('finance.transactions','finance.cash','finance.partners','finance.documents','finance.reports') THEN true ELSE false END,
      CASE WHEN m.code IN('finance.dashboard','finance.checkout','finance.transactions','finance.cash','finance.partners','finance.documents','finance.reports') THEN true ELSE false END,

 ...
 *
 * Normal finance requests are throttled for 30 seconds; settlement recovery can
 * force an immediate recheck so a newly created salon is never blocked by the cache.
 */
export async function ensureSalonDefaultLegalEntities(force=false){
  if(!force&&lastSuccessfulAt&&Date.now()-lastSuccessfulAt<CACHE_MS)return;
  ...
  pending=(async()=>{
    ...
    `)).rows[0]||{};
    if(!state.locations||!state.legal_entities||!state.legal_entity_locations){

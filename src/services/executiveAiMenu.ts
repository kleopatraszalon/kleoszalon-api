  ...
  try{
    ...
    await db.query(`WITH p AS (SELECT id FROM menus WHERE code='analytics' LIMIT 1)
      ...
      SELECT x.code,x.name,x.icon,x.route,x.order_index,p.id,'analytics',true FROM p CROSS JOIN (VALUES
        ...
        ('analytics.business_continuity_gameday','Üzletmenet-folytonossági GameDay','ShieldCheck','/finance/exception-command-center/gameday',22),
        ('analytics.operational_risk','Operational Risk & Control Register','Gauge','/finance/exception-command-center/risk-register',23)
    ...
    if(perms){
      await db.query(`INSERT INTO role_menu_permissions(
        ...
        CROSS JOIN menus m WHERE m.code IN('analytics','analytics.executive_ai','analytics.exception_center','analytics.exception_intelligence','analytics.exception_capa','analytics.capa_workqueue','analytics.major_incident','analytics.resilience_recovery','analytics.business_continuity_gameday','analytics.operational_risk')
      ...
      await db.query(`UPDATE role_menu_permissions p
        ...
        WHERE p.menu_id=m.id
          AND m.code IN('analytics.executive_ai','analytics.exception_center','analytics.exception_intelligence','analytics.exception_capa','analytics.capa_workqueue','analytics.major_incident','analytics.resilience_recovery','analytics.business_continuity_gameday','analytics.operational_risk')

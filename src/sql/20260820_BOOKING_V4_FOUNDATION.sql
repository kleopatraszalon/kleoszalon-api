...
INSERT INTO booking_staff_levels(code,name,sort_order)
VALUES
  ('trainee','Gyakornok',10),
  ('normal','Normál',20),
  ('top','TOP',30),
  ('master','Master',40)
...
CREATE TABLE IF NOT EXISTS employee_professional_categories (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY(employee_id, category_code)
);
...
CREATE TABLE IF NOT EXISTS booking_service_prices_by_level (
  ...
  staff_level_id uuid NOT NULL REFERENCES booking_staff_levels(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_booking_service_prices_by_level
  ON booking_service_prices_by_level(service_id, staff_level_id, COALESCE(location_id,'00000000-0000-0000-0000-000000000000'::uuid));
...
CREATE TABLE IF NOT EXISTS booking_service_recommendations (
  ...
  recommended_service_id uuid NOT NULL REFERENCES services(id) ON DELETE CASCADE,
  location_id uuid REFERENCES locations(id) ON DELETE CASCADE,
  priority int NOT NULL DEFAULT 100,
  recommendation_type text NOT NULL DEFAULT 'cross_sell' CHECK(recommendation_type IN ('upsell','cross_sell','bundle')),
  label text,
...
CREATE TABLE IF NOT EXISTS booking_coupon_campaigns (
  ...
  minimum_booking_value numeric(12,2),
  max_total_uses int,
  max_uses_per_customer int,
  combinable boolean NOT NULL DEFAULT false,
  exclude_last_minute boolean NOT NULL DEFAULT true,
  is_active boolean NOT NULL DEFAULT true,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);

CREATE TABLE IF NOT EXISTS booking_coupon_locations (
  coupon_id uuid NOT NULL REFERENCES booking_coupon_campaigns(id) ON DELETE CASCADE,
  location_id uuid NOT NULL REFERENCES locations(id) ON DELETE CASCADE,
  PRIMARY KEY(coupon_id, location_id)
);

CREATE TABLE IF NOT EXISTS booking_coupon_services (
  coupon_id uuid NOT NULL REFERENCES booking_coupon_campaigns(id) ON DELETE CASCADE,
  service_id uuid NOT NULL REFERENCES services(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS booking_last_minute_rules (
  ...
  staff_level_id uuid REFERENCES booking_staff_levels(id) ON DELETE SET NULL,
  ...
  validity_hours int NOT NULL DEFAULT 24 CHECK(validity_hours BETWEEN 1 AND 168),
...
CREATE TABLE IF NOT EXISTS booking_last_minute_offers (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  rule_id uuid REFERENCES booking_last_minute_rules(id) ON DELETE SET NULL,
  location_id uuid NOT NULL REFERENCES locations(id) ON DELETE CASCADE,
  service_id uuid NOT NULL REFERENCES services(id) ON DELETE CASCADE,
  employee_id uuid NOT NULL REFERENCES employees(id) ON DELETE CASCADE,
  ...
  CHECK(offer_price >= 0 AND original_price >= offer_price)
);
CREATE UNIQUE INDEX IF NOT EXISTS uq_booking_last_minute_offer_slot
  ON booking_last_minute_offers(location_id, service_id, employee_id, start_time)
  WHERE status='active';
...
CREATE TABLE IF NOT EXISTS booking_marketing_consents (
  ...
  consent_text_version text NOT NULL DEFAULT 'booking-v4-2026-08-20',

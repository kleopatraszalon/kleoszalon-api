...
SET name = 'Szolgáltatási kategóriák',
    route = '/masterdata/services?view=categories',
...
WITH parent AS (SELECT id FROM menus WHERE code = 'masterdata' LIMIT 1)
INSERT INTO menus(code,name,icon,route,order_index,parent_id,feature_key,is_active)
SELECT 'masterdata.employee-services','Szakember–szolgáltatás beállítások',NULL,'/masterdata/services?view=staff',42,parent.id,'employee_services',true FROM parent
ON CONFLICT(code) DO UPDATE SET name=EXCLUDED.name, route=EXCLUDED.route, order_index=EXCLUDED.order_index,
  parent_id=EXCLUDED.parent_id, feature_key=EXCLUDED.feature_key, is_active=true;

...
BEGIN
  ...
  IF NEW.legal_entity_id IS NULL THEN
    BEGIN location_value:=NEW.location_id::text; EXCEPTION WHEN undefined_column THEN location_value:=NULL; END;
    IF NULLIF(location_value,'') IS NOT NULL THEN
      SELECT el.legal_entity_id INTO NEW.legal_entity_id FROM legal_entity_locations el JOIN legal_entities e ON e.id=el.legal_entity_id WHERE el.location_id::text=location_value AND el.active=true AND e.active=true ORDER BY el.is_default DESC,e.created_at LIMIT 1;
    END IF;
  END IF;
  RETURN NEW;
END $$;
...
DO $$ BEGIN
 IF to_regclass('public.retail_sales') IS NOT NULL THEN
  EXECUTE $q$CREATE OR REPLACE FUNCTION vir_fill_retail_legal_entity() RETURNS trigger LANGUAGE plpgsql AS $f$
  BEGIN
   IF NEW.legal_entity_id IS NULL THEN SELECT el.legal_entity_id INTO NEW.legal_entity_id FROM legal_entity_locations el JOIN legal_entities e ON e.id=el.legal_entity_id WHERE el.location_id::text=NEW.location_id::text AND el.active=true AND e.active=true ORDER BY el.is_default DESC,e.created_at LIMIT 1; END IF;
   IF NEW.legal_entity_id IS NULL THEN RAISE EXCEPTION 'A termékeladáshoz nincs kibocsátó cég.' USING ERRCODE='23514'; END IF;
   RETURN NEW;
  END $f$$q$;
...
DO $$ BEGIN
 IF to_regclass('public.vir_receipts') IS NOT NULL THEN
  EXECUTE $q$CREATE OR REPLACE FUNCTION vir_fill_receipt_legal_entity() RETURNS trigger LANGUAGE plpgsql AS $f$
  BEGIN
   IF NEW.legal_entity_id IS NULL AND NEW.original_receipt_id IS NOT NULL THEN SELECT legal_entity_id INTO NEW.legal_entity_id FROM vir_receipts WHERE id=NEW.original_receipt_id; END IF;
   IF NEW.legal_entity_id IS NULL AND NEW.source_type='WORK_ORDER' THEN SELECT legal_entity_id INTO NEW.legal_entity_id FROM work_orders WHERE id::text=NEW.source_id; END IF;
   IF NEW.legal_entity_id IS NULL AND NEW.source_type='RETAIL_SALE' AND to_regclass('public.retail_sales') IS NOT NULL THEN EXECUTE 'SELECT legal_entity_id FROM retail_sales WHERE id::text=$1' INTO NEW.legal_entity_id USING NEW.source_id; END IF;
   RETURN NEW;
  END $f$$q$;

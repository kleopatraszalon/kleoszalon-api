...
CREATE UNIQUE INDEX IF NOT EXISTS uq_finance_payment_methods_scope
  ON finance_payment_methods(COALESCE(location_id,''),code);
...
CREATE INDEX IF NOT EXISTS cashier_shift_counts_shift_idx
  ON cashier_shift_counts(shift_id,created_at DESC);
CREATE INDEX IF NOT EXISTS cashier_shift_counts_location_date_idx
  ON cashier_shift_counts(location_id,business_date DESC,created_at DESC);
...
CREATE INDEX IF NOT EXISTS cashier_payment_context_lookup_idx
  ON cashier_payment_context(work_order_id,consumed_at,expires_at,sequence_no);
...
CREATE INDEX IF NOT EXISTS work_order_payment_refunds_payment_idx
  ON work_order_payment_refunds(payment_id,created_at DESC);
CREATE INDEX IF NOT EXISTS work_order_payment_refunds_workorder_idx
  ON work_order_payment_refunds(work_order_id,created_at DESC);
...
BEGIN
  ...
  IF to_regclass('public.cash_register_shifts') IS NOT NULL THEN
    EXECUTE 'SELECT id FROM cash_register_shifts WHERE location_id=$1 AND status=''open'' ORDER BY opened_at DESC LIMIT 1'
      INTO v_shift USING v_location;
  END IF;
  ...
  SELECT * INTO v_ctx
  FROM cashier_payment_context
  WHERE work_order_id=NEW.work_order_id::text
    AND consumed_at IS NULL
    AND expires_at>now()
    AND base_method=NEW.payment_method

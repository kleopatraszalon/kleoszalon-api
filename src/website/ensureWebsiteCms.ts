...
export async function ensureWebsiteCms() {
  ...
  const menus = await pool.query(`
    WITH input(code,name,icon,route,order_index) AS (
      VALUES
        ('settings.website','Weboldal admin','Globe2','/admin/website',170),
        ('settings.website_pages','Weboldal tartalom','BookOpenText','/admin/website/pages',171)
    )
    INSERT INTO menus(code,name,icon,route,order_index,parent_id,feature_key,is_active)
    SELECT code,name,icon,route,order_index,$1,'marketing',true FROM input

...
async function guard(req:AuthRequest,res:Response,next:NextFunction){
 try{
  ...
  if(!tenant){const denied=(req.user as any)?.tenant_feature_denied;return res.status(403).json({error:denied?`A(z) ${denied} modul nincs engedélyezve az előfizetési csomagban.`:'A felhasználóhoz nincs aktív tenant-hozzáférés.',code:denied?'TENANT_FEATURE_DISABLED':'TENANT_ACCESS_DENIED',feature:denied||undefined});}
  ...
  const own=ownLocation(req);if(own&&!(await locationBelongsToTenant(own,tenant.id)))return res.status(403).json({error:'A felhasználó telephelye nem az aktív tenanthoz tartozik.',code:'TENANT_LOCATION_MISMATCH'});
  ...
  if(req.method==='GET'&&path==='/timesheets'){
   ...
   const {rows}=await db.query(`SELECT t.*,e.full_name,l.name location_name FROM timesheets t JOIN employees e ON e.id=t.employee_id LEFT JOIN locations l ON l.id=COALESCE(t.location_id,e.location_id) WHERE e.tenant_id=$5::bigint AND ($1::date IS NULL OR t.work_date >= $1) AND ($2::date IS NULL OR t.work_date <= $2) AND ($3::uuid IS NULL OR t.employee_id=$3) AND COALESCE(t.location_id,e.location_id)=$4::uuid ORDER BY t.work_date DESC,e.full_name`,[from,to,employee,locationId,tenant.id]);return res.json(rows)
  }
  ...
  if(req.method==='POST'&&path==='/timesheets'){
   if(!(await employeeBelongs(req.body?.employee_id,locationId,tenant.id)))return res.status(403).json({error:'Csak a saját üzlet munkatársának rögzíthető jelenlét.'});if(!req.body||typeof req.body!=='object')req.body={};req.body.location_id=locationId;return next()
  }

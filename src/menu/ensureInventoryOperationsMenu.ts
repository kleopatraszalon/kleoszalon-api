...
export async function ensureInventoryOperationsMenu(){
  ...
  await safe(`WITH p AS (SELECT id FROM menus WHERE code='inventory' LIMIT 1)
    INSERT INTO menus(code,name,icon,route,order_index,parent_id,feature_key,is_active)
    SELECT 'inventory.lots','Sarzs és lejárat (FEFO)','Clock','/warehouse/lots',31,p.id,'inventory',true FROM p
    ON CONFLICT(code) DO UPDATE SET name=EXCLUDED.name,icon=EXCLUDED.icon,route=EXCLUDED.route,order_index=EXCLUDED.order_index,parent_id=EXCLUDED.parent_id,feature_key='inventory',is_active=true`);
  ...
  await safe(`INSERT INTO role_menu_permissions(role_key,menu_id,can_view,can_create,can_edit,can_delete,can_approve,can_export,can_view_financial,can_manage_permissions,scope_type,updated_at)
    SELECT 'admin',m.id,true,true,true,true,true,true,false,true,'all_locations',now() FROM menus m WHERE m.code IN('inventory.operations','inventory.lots')
  ...
  await safe(`INSERT INTO role_menu_permissions(role_key,menu_id,can_view,can_create,can_edit,can_delete,can_approve,can_export,can_view_financial,can_manage_permissions,scope_type,updated_at)
    SELECT 'manager',m.id,true,true,true,false,true,true,false,false,'all_locations',now() FROM menus m WHERE m.code IN('inventory.operations','inventory.lots')
  ...
  await safe(`INSERT INTO role_menu_permissions(role_key,menu_id,can_view,can_create,can_edit,can_delete,can_approve,can_export,can_view_financial,can_manage_permissions,scope_type,updated_at)
    ...
    FROM (VALUES('location_manager'),('salon_manager'),('receptionist')) r(role_key) CROSS JOIN menus m WHERE m.code IN('inventory.operations','inventory.lots')
  ...
  await safe(`INSERT INTO role_menu_permissions(role_key,menu_id,can_view,can_create,can_edit,can_delete,can_approve,can_export,can_view_financial,can_manage_permissions,scope_type,updated_at)
    ...
    FROM (VALUES('employee'),('customer')) r(role_key) CROSS JOIN menus m WHERE m.code IN('inventory.operations','inventory.lots')

...
export async function ensureMenuHealth(){
  ...
  await safe(`UPDATE menus SET is_active=true WHERE code IN(
    'dashboard','appointments','appointments.workorders','finance','finance.dashboard','finance.checkout','finance.cash','customers.loyalty_program','customers.forms','customers.duplicate_review',
  ...
  await safe(`INSERT INTO role_menu_permissions(role_key,menu_id,can_view,can_create,can_edit,can_delete,can_approve,can_export,can_view_financial,can_manage_permissions,scope_type,updated_at)
    SELECT 'admin',m.id,true,true,true,true,true,true,true,true,'all_locations',now() FROM menus m WHERE m.code IN('appointments.workorders','finance.nav_online_invoice','procurement.central_supply','customers.forms','customers.duplicate_review')
  ...
  await safe(`INSERT INTO role_menu_permissions(role_key,menu_id,can_view,can_create,can_edit,can_delete,can_approve,can_export,can_view_financial,can_manage_permissions,scope_type,updated_at)
    ...
    FROM (VALUES('manager'),('location_manager'),('salon_manager')) r(role_key) CROSS JOIN menus m WHERE m.code IN('customers.forms','customers.duplicate_review')
  ...
  await safe(`INSERT INTO role_menu_permissions(role_key,menu_id,can_view,can_create,can_edit,can_delete,can_approve,can_export,can_view_financial,can_manage_permissions,scope_type,updated_at)
    SELECT 'receptionist',m.id,true,false,false,false,false,false,false,false,'own_location',now() FROM menus m WHERE m.code IN('customers.forms','customers.duplicate_review')

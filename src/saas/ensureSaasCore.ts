 ...
 *
 * IMPORTANT: schema DDL must never compete on ordinary API requests. A dedicated
 * pool client and a PostgreSQL advisory lock serialize bootstrap work across
 * Render instances, while a longer bootstrap-only statement timeout prevents
 * the normal 8s API timeout from aborting ALTER/CREATE operations halfway.
 */
export function ensureSaasCore(): Promise<void> {
  ...
  ensurePromise=(async()=>{
    ...
    try{
      ...
      await client.query("SELECT pg_advisory_lock($1,$2)",[20260816,1]);
      advisoryLocked=true;
      ...
      if(advisoryLocked)await client.query("SELECT pg_advisory_unlock($1,$2)",[20260816,1]).catch(()=>{});

...
function isVirRequest(req: AuthRequest): boolean {
  const path = requestPath(req);
...
async function tenantFromAuthenticatedLocation(userId: string, locationId: unknown, role: unknown) {
  ...
  const { rows } = await db.query(
    `SELECT t.id::text id,t.slug,COALESCE(tu.tenant_role,$3::text) tenant_role
       FROM locations l
...
async function tenantFromToken(userId: string, tenantId: string) {
  ...
  const { rows } = await db.query(
    `SELECT t.id::text id,t.slug,tu.tenant_role
...
export async function resolveTenantIdentity(req: AuthRequest): Promise<TenantIdentity | null> {
  ...
  const authUser = req.user as (NonNullable<AuthRequest["user"]> & {
    tenant_id?: string | number | null;
    tenant_feature_denied?: string | null;
  }) | undefined;
  ...
  // Multi-tenant resolution is deliberately fail-closed. There is no default
  // Kleopatra fallback: missing explicit ownership means no tenant identity.
  if (!row) return null;
...
export async function tenantLocationIds(tenantId: string): Promise<string[]> {
  const { rows } = await db.query(`SELECT id::text id FROM locations WHERE tenant_id=$1`, [tenantId]);
...
export async function locationBelongsToTenant(locationId: unknown, tenantId: string): Promise<boolean> {
  ...
  const { rows } = await db.query(`SELECT 1 FROM locations WHERE id::text=$1 AND tenant_id=$2 LIMIT 1`, [value, tenantId]);
...
export async function entityBelongsToTenant(table: string, id: string, tenantId: string): Promise<boolean> {
  ...
  const hasLocation = await db.query(
    `SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name=$1 AND column_name='location_id' LIMIT 1`,
    [table],
  );
  const sql = hasLocation.rowCount
    ? `SELECT 1 FROM ${table} e LEFT JOIN locations l ON l.id::text=e.location_id::text WHERE e.id::text=$1 AND (e.tenant_id::text=$2::text OR l.tenant_id::text=$2::text) LIMIT 1`
    : `SELECT 1 FROM ${table} e WHERE e.id::text=$1 AND e.tenant_id::text=$2::text LIMIT 1`;
...
export async function tenantFeatureEnabled(tenantId: string, featureKey: string): Promise<boolean> {
  const { rows } = await db.query(
    `SELECT COALESCE(tf.enabled,CASE WHEN COALESCE((sp.features->>'all_modules')::boolean,false) THEN true WHEN sp.features ? $2 THEN COALESCE((sp.features->>$2)::boolean,false) ELSE false END) AS enabled
       FROM tenants t

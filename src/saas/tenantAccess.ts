...
export async function resolveTenantIdentity(req: AuthRequest): Promise<TenantIdentity|null> {
  ...
  // New tokens with an explicit tenant are authoritative. Legacy admin/staff tokens,
  // however, commonly carry a signed location_id without tenant_id. In that case the
  // selected salon is the strongest tenant signal and must be resolved before a
  // possibly unrelated tenant_users membership or the historical Kleopatra fallback.

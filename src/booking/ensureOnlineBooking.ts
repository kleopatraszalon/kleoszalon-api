...
/**
 * Online-booking schema bootstrap is intentionally single-flight per API process.
 * Several public routes and async communication workers call this helper. Running
 * its DDL-heavy, idempotent bootstrap concurrently can race on DROP/ADD constraint
 * and DROP/CREATE trigger statements. Once a process initialized the schema,
 * request paths only reuse the ready state; failures reset the in-flight promise
 * so a later request can retry safely.
 */

...
async function aiCopy(selected:string[],items:BookingRecommendation[]){
  const key=String(process.env.OPENAI_API_KEY||'').trim();if(!key||!items.length){lastAiStatus='not_configured';return null}if(aiBackoffUntil>Date.now()){lastAiStatus='rate_limited';return null}

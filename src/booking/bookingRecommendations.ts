...
async function aiCopy(selected:string[],items:BookingRecommendation[]){
  ...
  try{
    const response=await axios.post('https://api.openai.com/v1/responses',{model:process.env.BOOKING_RECOMMENDATION_MODEL||process.env.OPENAI_MODEL||'gpt-5-mini',store:false,max_output_tokens:260,input:[{role:'system',content:[{type:'input_text',text:'Magyar szépségszalon foglalási ajánlószöveget írsz. Kizárólag a kapott service_id-kat használd. Árat, kedvezményt, hatást vagy elérhetőséget ne találj ki. JSON tömböt adj: [{"service_id":"...","title":"...","message":"..."}]. A cím max. 45, az üzenet max. 150 karakter.'}]},{role:'user',content:[{type:'input_text',text:JSON.stringify({selected_services:selected,candidates:allowed})}]}]},{headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},timeout:4500});

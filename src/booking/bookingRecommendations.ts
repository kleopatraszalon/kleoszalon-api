...
async function aiCopy(selected:string[],items:BookingRecommendation[]){
  ...
  try{
    const response=await axios.post('https://api.openai.com/v1/responses',{model:process.env.BOOKING_RECOMMENDATION_MODEL||process.env.OPENAI_MODEL||'gpt-5-mini',store:false,max_output_tokens:220,reasoning:{effort:'minimal'},text:{verbosity:'low',format:{type:'json_schema',name:'booking_recommendations',strict:true,schema:{type:'object',properties:{recommendations:{type:'array',items:{type:'object',properties:{service_id:{type:'string'},title:{type:'string',maxLength:45},message:{type:'string',maxLength:150}},required:['service_id','title','message'],additionalProperties:false}}},required:['recommendations'],additionalProperties:false}}},input:[{role:'system',content:[{type:'input_text',text:'Magyar szépségszalon foglalási ajánlószöveget írsz. Kizárólag a kapott service_id-kat használd. Árat, kedvezményt, hatást vagy elérhetőséget ne találj ki.'}]},{role:'user',content:[{type:'input_text',text:JSON.stringify({selected_services:selected,candidates:allowed})}]}]},{headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},timeout:6500});

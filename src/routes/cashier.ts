...
async function orderFinancials(client: any, workOrderId: string) {
  const items = await client.query(
    `SELECT COALESCE(SUM(line_total),0)::numeric AS gross_total
     FROM work_order_items WHERE work_order_id = $1`,
    [workOrderId]
  );
  const payments = await client.query(
    `SELECT COALESCE(SUM(amount),0)::numeric AS amount_paid
     FROM work_order_payments WHERE work_order_id = $1`,
    [workOrderId]
  );
...
router.get("/workorders", async (req: AuthRequest, res, next) => {
  try {
    ...
    if (locationId) {
      ...
      where += ` AND wo.location_id::text = $${params.length}`;
    }
    const { rows } = await db.query(
      `SELECT wo.id, wo.created_at, wo.completed_at, wo.status, wo.client_name, wo.employee_id,
       ...
       FROM work_orders wo
       LEFT JOIN LATERAL (
         SELECT SUM(line_total)::numeric AS gross_total FROM work_order_items wi WHERE wi.work_order_id = wo.id
       ) i ON true
       LEFT JOIN LATERAL (
         SELECT SUM(amount)::numeric AS paid_total FROM work_order_payments wp WHERE wp.work_order_id = wo.id
       ) p ON true
       ${where}
       ORDER BY COALESCE(wo.completed_at, wo.created_at) DESC
       LIMIT 200`,
      params
    );
    res.json(rows);
...
router.get("/workorders/:id", async (req: AuthRequest, res, next) => {
  try {
    ...
    const items = await db.query(
      `SELECT id,item_type,item_name,quantity,unit_price,discount_amount,line_total
       FROM work_order_items WHERE work_order_id=$1 ORDER BY created_at`,
      [req.params.id]
    );
    const payments = await db.query(
      `SELECT id,payment_method,amount,paid_at,note FROM work_order_payments
       WHERE work_order_id=$1 ORDER BY paid_at`,
      [req.params.id]
    );
...
router.post("/workorders/:id/settle", async (req: AuthRequest, res, next) => {
  ...
  try {
    ...
    for (const p of incomingPayments) {
      ...
      await client.query(
        `INSERT INTO work_order_payments (work_order_id,payment_method,amount,paid_at,note)
         VALUES ($1,$2,$3,now(),$4)`,
        [req.params.id, method, amount, p?.note || null]
      );
    }
    ...
    if (closeFinancially && paymentStatus !== "paid") {
      throw new Error(`A munkalap nem zárható pénzügyileg: még ${money(amountDue - amountPaid)} Ft fizetendő.`);
    }
    ...
    const updated = await client.query(
      `UPDATE work_orders
       SET gross_total=$2, discount_amount=$3, tip_amount=$4, amount_due=$5, amount_paid=$6,
           ...
           financial_closed_at=CASE WHEN $9 THEN now() ELSE financial_closed_at END,
           financial_closed_by=CASE WHEN $9 THEN $10 ELSE financial_closed_by END,
           updated_at=now()
       WHERE id=$1 RETURNING *`,
      [req.params.id, calc.grossTotal, discount, tip, amountDue, amountPaid, paymentStatus,
       invoiceStatus, closeFinancially, req.user?.email || String(req.user?.id || "")]
    );
    ...
    if (message.startsWith("Érvénytelen") || message.startsWith("A fizetési") || message.startsWith("A munkalap nem zárható")) {
  ...
  try {
    ...
    const { rows } = await db.query(
      `SELECT
       ...
       WHERE wp.paid_at::date=$1::date ${locationFilter}`,
      params
    );
...
router.post("/daily-close", async (req: AuthRequest, res, next) => {
  try {
    ...
    const openingCash = money(req.body?.opening_cash);
    ...
    const tipDiscount = await db.query(
      `SELECT COALESCE(SUM(tip_amount),0)::numeric tips, COALESCE(SUM(discount_amount),0)::numeric discounts
       FROM work_orders wo WHERE financial_closed_at::date=$1::date ${locationFilter}`,
      params
    );
    const expectedCash = money(openingCash + money(t.cash_sales));
    ...
    const { rows } = await db.query(
      `INSERT INTO cash_register_closings
       ...
       RETURNING *`,
      [locationId,businessDate,openingCash,money(t.cash_sales),money(t.card_sales),money(t.transfer_sales),money(t.voucher_sales),money(t.other_sales),
       money(tipDiscount.rows[0]?.tips),money(tipDiscount.rows[0]?.discounts),expectedCash,countedCash,difference,req.body?.note || null,req.user?.email || String(req.user?.id || "")]
    );
...
router.get("/daily-closes", async (req: AuthRequest, res, next) => {
  try {
    const { rows } = await db.query(`SELECT * FROM cash_register_closings ORDER BY business_date DESC, closed_at DESC LIMIT 100`);
    res.json(rows);
  } catch (err) { next(err); }
});

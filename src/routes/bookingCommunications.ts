...
router.put("/settings",async(req:AuthRequest,res,next)=>{try{const locationId=String(req.body?.location_id||req.user?.location_id||"").trim();if(!locationId)return res.status(400).json({error:"location_id kötelező."});const b=req.body||{};const{rows}=await db.query(`INSERT INTO booking_communication_settings(location_id,confirmation_enabled,reminder_48h_enabled,reminder_24h_enabled,cancellation_enabled,waitlist_enabled,review_request_enabled,review_delay_hours,updated_at) VALUES($1::uuid,$2,$3,$4,$5,$6,$7,$8,now()) ON CONFLICT(location_id) DO UPDATE SET confirmation_enabled=EXCLUDED.confirmation_enabled,reminder_48h_enabled=EXCLUDED.reminder_48h_enabled,reminder_24h_enabled=EXCLUDED.reminder_24h_enabled,cancellation_enabled=EXCLUDED.cancellation_enabled,waitlist_enabled=EXCLUDED.waitlist_enabled,review_request_enabled=EXCLUDED.review_request_enabled,review_delay_hours=EXCLUDED.review_delay_hours,updated_at=now() RETURNING *`,[locationId,b.confirmation_enabled!==false,b.reminder_48h_enabled!==false,b.reminder_24h_enabled!==false,b.cancellation_enabled!==false,b.waitlist_enabled!==false,b.review_request_enabled!==false,Math.max(0,Number(b.review_delay_hours||24))]);res.json(rows[0]);}catch(err){next(err)}});

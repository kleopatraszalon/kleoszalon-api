...
router.get("/failure-analysis",async(req:AuthRequest,res,next)=>{try{
  ...
  const{rows}=await db.query(`SELECT
      ...
      q.created_at,q.failed_at,l.name location_name
  ...
  res.json({
    ...
    provider_status:{
      email:{enabled:process.env.DISABLE_SMTP!=="1",configured:Boolean(process.env.SMTP_USER&&process.env.SMTP_PASS)},
    ...
    safety:{bulk_requeue_allowed:false,message:"A sikertelen értesítések automatikus tömeges újraküldése tiltott. Előbb az okot és az érintett foglalások aktualitását kell ellenőrizni."}
  });
}catch(err){next(err)}});

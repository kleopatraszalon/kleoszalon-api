  ...
  try{
    ...
    const {rows}=await pool.query(`
      WITH wo_base AS (
        ...
        FROM work_order_items wi JOIN wo_base w ON w.id=wi.work_order_id GROUP BY wi.work_order_id
        ...
        SELECT 'COMPLETED_UNPAID'::text leakage_type,'CRITICAL'::text severity,w.id::text entity_id,'work_order'::text entity_type,
          w.location_id,w.event_time,w.work_order_number title,
          GREATEST(w.amount_due,CASE WHEN w.payment_status NOT IN('paid','settled','completed') THEN w.gross_total ELSE 0 END)::numeric estimated_loss,
          jsonb_build_object('gross_total',w.gross_total,'amount_due',w.amount_due,'payment_status',w.payment_status) details
        FROM wo_base w WHERE lower(COALESCE(w.status,''))='completed' AND (w.amount_due>0 OR (w.gross_total>0 AND w.payment_status NOT IN('paid','settled','completed','')))
        UNION ALL
        ...
        FROM wo_base w WHERE lower(COALESCE(w.status,''))='completed' AND w.gross_total<=0
        UNION ALL
        ...
        FROM wo_base w JOIN item_totals i ON i.work_order_id=w.id
        ...
        FROM wo_base w LEFT JOIN item_totals i ON i.work_order_id=w.id

...
async function ensureCurrentStageMenus(){
 ...
 await pool.query(`INSERT INTO menus(code,name,icon,route,order_index,parent_id,feature_key,is_active) VALUES('settings.security','Biztonsági beállítások','ShieldCheck','/admin/security-settings',185,$1,'audit',true),('settings.system_health','Rendszerellenőrzés','Activity','/admin/system-health',190,$1,'audit',true),('settings.uat','UAT tesztközpont','ClipboardCheck','/admin/uat',195,$1,'audit',true) ON CONFLICT(code) DO UPDATE SET name=EXCLUDED.name,icon=EXCLUDED.icon,route=EXCLUDED.route,order_index=EXCLUDED.order_index,parent_id=EXCLUDED.parent_id,feature_key=EXCLUDED.feature_key,is_active=true`,[settingsId]);
 ...
 await pool.query(`INSERT INTO role_menu_permissions(role_key,menu_id,can_view,can_create,can_edit,can_delete,can_approve,can_export,can_view_financial,can_manage_permissions,scope_type,updated_at) SELECT 'admin',m.id,true,true,true,false,false,false,false,true,'all_locations',now() FROM menus m WHERE m.code='settings.security' ON CONFLICT(role_key,menu_id) DO UPDATE SET can_view=true,can_create=true,can_edit=true,can_delete=false,can_approve=false,can_export=false,can_view_financial=false,can_manage_permissions=true,scope_type='all_locations',updated_at=now()`).catch(()=>undefined);
}

...
async function ensureProcurementMenu(){
  await pool.query(`
    ...
    ON CONFLICT(code) DO UPDATE SET name=EXCLUDED.name,icon=EXCLUDED.icon,route=EXCLUDED.route,order_index=EXCLUDED.order_index,parent_id=NULL,feature_key='inventory',is_active=true
  `);
  await pool.query(`
  ...
  await pool.query(`
    ...
    SELECT rp.role_key, target.id, rp.can_view,rp.can_create,rp.can_edit,rp.can_delete,rp.can_approve,rp.can_export,rp.can_view_financial,rp.can_manage_permissions,rp.scope_type

...
router.get("/availability",async(req:Request,res:Response)=>{
  try{
    ...
    const busy=(await db.query(`SELECT employee_id::text employee_id,kleo_booking_utc(start_time) start_time,kleo_booking_utc(end_time) end_time FROM appointments WHERE ($5::uuid IS NULL OR id<>$5::uuid) AND location_id=$1::uuid AND employee_id=ANY($2::uuid[]) AND status NOT IN('cancelled','canceled','no_show') AND start_time<$4::timestamptz AND end_time>$3::timestamptz UNION ALL SELECT employee_id::text employee_id,kleo_booking_utc(start_time) start_time,kleo_booking_utc(end_time) end_time FROM appointment_technical_breaks WHERE location_id=$1::uuid AND employee_id=ANY($2::uuid[]) AND start_time<$4::timestamptz AND end_time>$3::timestamptz`,[locationId,employeeIds,bounds.from.toISOString(),bounds.to.toISOString(),excludeAppointmentId])).rows;

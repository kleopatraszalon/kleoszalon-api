...
async function validateBookSchedule(req:Request,res:Response,next:NextFunction){
  ...
  try{
    ...
    const local=(await db.query(`SELECT ($1::timestamptz AT TIME ZONE 'Europe/Budapest')::date AS booking_day,EXTRACT(HOUR FROM ($1::timestamptz AT TIME ZONE 'Europe/Budapest'))::int*60+EXTRACT(MINUTE FROM ($1::timestamptz AT TIME ZONE 'Europe/Budapest'))::int AS start_min,($2::timestamptz AT TIME ZONE 'Europe/Budapest')::date AS end_day,EXTRACT(HOUR FROM ($2::timestamptz AT TIME ZONE 'Europe/Budapest'))::int*60+EXTRACT(MINUTE FROM ($2::timestamptz AT TIME ZONE 'Europe/Budapest'))::int AS end_min`,[start.toISOString(),end.toISOString()])).rows[0];
    if(String(local.booking_day)!==String(local.end_day)||Number(local.start_min)<Number(cfg.opening_minute||480)||Number(local.end_min)>Number(cfg.closing_minute||1200))return res.status(409).json({error:"A kiválasztott időpont a szalon online foglalási nyitvatartásán kívül esik."});

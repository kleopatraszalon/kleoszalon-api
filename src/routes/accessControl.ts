...
function rolesOf(req:AuthRequest){return parseRoleKeys(req.user?.role)}
...
router.get("/me/features",asyncRoute(async(req:AuthRequest,res)=>{
  ...
  const {rows}=await pool.query(`SELECT role_key,feature_key,can_use,scope_type FROM role_feature_permissions WHERE lower(role_key)=ANY($1::text[])`,[roles]);
  const grouped=new Map<string,any[]>();
  for(const row of rows){const list=grouped.get(row.feature_key)||[];list.push(row);grouped.set(row.feature_key,list)}
  res.json({admin:false,features:FEATURE_DEFINITIONS.map(f=>{const list=grouped.get(f.feature_key)||[];const allowed=list.filter(x=>x.can_use);return {...f,configured:list.length>0,can_use:allowed.length>0,scope_type:strongestScope(allowed.map(x=>String(x.scope_type||"own_location")))}})});
}));
...
router.get("/me/capabilities",asyncRoute(async(req:AuthRequest,res)=>{
  ...
  if(admin){
    return res.json({admin:true,roles,allowed_location_ids:locations.map((x:any)=>String(x.id)),features:Object.fromEntries(FEATURE_DEFINITIONS.map(f=>[f.feature_key,{can_use:true,scope_type:"all_locations"}])),menus:Object.fromEntries(menus.map(m=>[m.code,{menu_id:m.id,code:m.code,configured:true,can_view:true,can_create:true,can_edit:true,can_delete:true,can_approve:true,can_export:true,can_view_financial:true,can_manage_permissions:true,scope_type:"all_locations"}]))});
  }
  const [fp,mp,lp]=await Promise.all([
    pool.query(`SELECT feature_key,can_use,scope_type FROM role_feature_permissions WHERE lower(role_key)=ANY($1::text[])`,[roles]),
    pool.query(`SELECT p.*,m.code FROM role_menu_permissions p JOIN menus m ON m.id=p.menu_id WHERE lower(p.role_key)=ANY($1::text[]) AND COALESCE(m.is_active,true)=true`,[roles]),
    pool.query(`SELECT location_id FROM role_location_permissions WHERE lower(role_key)=ANY($1::text[]) AND can_access=true`,[roles])
  ]);
  const features:Record<string,any>={};
  for(const f of FEATURE_DEFINITIONS){const list=fp.rows.filter((x:any)=>x.feature_key===f.feature_key);const allowed=list.filter((x:any)=>x.can_use);features[f.feature_key]={configured:list.length>0,can_use:allowed.length>0,scope_type:strongestScope(allowed.map((x:any)=>String(x.scope_type||"own_location")))};}
  const menuCaps:Record<string,any>={};
  for(const menu of menus){const list=mp.rows.filter((x:any)=>Number(x.menu_id)===Number(menu.id));if(!list.length){menuCaps[menu.code]={menu_id:menu.id,code:menu.code,configured:false,can_view:false,can_create:false,can_edit:false,can_delete:false,can_approve:false,can_export:false,can_view_financial:false,can_manage_permissions:false,scope_type:"own_location"};continue;}const keys=["can_view","can_create","can_edit","can_delete","can_approve","can_export","can_view_financial","can_manage_permissions"];const cap:any={menu_id:menu.id,code:menu.code,configured:true};for(const key of keys)cap[key]=list.some((x:any)=>x[key]===true);cap.scope_type=strongestScope(list.filter((x:any)=>x.can_view).map((x:any)=>String(x.scope_type||"own_location")));menuCaps[menu.code]=cap;}

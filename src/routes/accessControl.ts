...
// Egyetlen kliensoldali lekérésből megadja a felhasználó tényleges funkció- és
// műveleti jogosultságait. Több szerepkörnél az engedélyek összeadódnak.
router.get("/me/capabilities",asyncRoute(async(req:AuthRequest,res)=>{
  ...
  for(const menu of menus){
    const list=mp.rows.filter((x:any)=>Number(x.menu_id)===Number(menu.id));
    // Ha nincs explicit szabály, null configured jelzi a kliensnek a visszafelé kompatibilis állapotot.
    if(!list.length){menuCaps[menu.code]={menu_id:menu.id,code:menu.code,configured:false,can_view:true,can_create:true,can_edit:true,can_delete:true,can_approve:true,can_export:true,can_view_financial:true,can_manage_permissions:false,scope_type:"own_location"};continue;}
    const keys=["can_view","can_create","can_edit","can_delete","can_approve","can_export","can_view_financial","can_manage_permissions"];
    const cap:any={menu_id:menu.id,code:menu.code,configured:true};
    for(const key of keys)cap[key]=list.some((x:any)=>x[key]===true);
    cap.scope_type=strongestScope(list.filter((x:any)=>x.can_view).map((x:any)=>String(x.scope_type||"own_location")));
    menuCaps[menu.code]=cap;
  }
  res.json({admin:false,roles,features,menus:menuCaps});
}));
...
router.put("/roles/:roleKey/features",asyncRoute(async(req,res)=>{
  ...
  try{
    ...
    for(const p of updates){const key=String(p.feature_key||"");if(!allowed.has(key))continue;await client.query(`INSERT INTO role_feature_permissions(role_key,feature_key,can_use,scope_type,updated_at) VALUES($1,$2,$3,$4,now()) ON CONFLICT(role_key,feature_key) DO UPDATE SET can_use=EXCLUDED.can_use,scope_type=EXCLUDED.scope_type,updated_at=now()`,[roleKey,key,!!p.can_use,p.scope_type||"own_location"]);}

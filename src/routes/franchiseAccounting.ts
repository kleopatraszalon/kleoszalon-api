...
router.get('/receivables',async(req:TenantAuthRequest,res:Response)=>{
  ...
  const params:any[]=[req.tenant!.id];let periodFilter='';
...
router.put('/members/:memberId/billing',adminOnly,async(req:TenantAuthRequest,res:Response)=>{
  ...
  try{
    const q=await db.query(`UPDATE franchise_members SET billing_legal_name=$3,billing_tax_number=$4,billing_email=$5,billing_country_code=$6,billing_postal_code=$7,billing_city=$8,billing_address=$9,billing_vat_rate=$10,updated_at=now() WHERE id=$1::bigint AND tenant_id=$2::bigint AND member_type='franchise' RETURNING id::text,location_id,billing_legal_name,billing_tax_number,billing_email,billing_country_code,billing_postal_code,billing_city,billing_address,billing_vat_rate`,[memberId,req.tenant!.id,String(req.body?.billing_legal_name||'').trim()||null,String(req.body?.billing_tax_number||'').trim()||null,String(req.body?.billing_email||'').trim()||null,String(req.body?.billing_country_code||'HU').trim().toUpperCase()||null,String(req.body?.billing_postal_code||'').trim()||null,String(req.body?.billing_city||'').trim()||null,String(req.body?.billing_address||'').trim()||null,vat]);
    if(!q.rowCount)return res.status(404).json({ok:false,error:'A franchise tag nem található ennél a tenantnál.'});return res.json({ok:true,row:q.rows[0]});
  }catch(error){console.error('[FRANCHISE-ACCOUNTING] billing',error);return res.status(500).json({ok:false,error:'A partner számlázási adatai nem menthetők.'});}
});

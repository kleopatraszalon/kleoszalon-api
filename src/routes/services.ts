...
import pool from "../db";
import servicesImportRouter from "./servicesImport";

const router = Router();

// Az Altegio import saját routerben van, hogy a nagy Excel-import egyetlen
// adatbázis-kapcsolaton, valódi tranzakcióban fusson.
...
router.get("/", async (req: Request, res: Response) => {
  ...
  const sql = `
    SELECT
      s.id, s.name, s.code, s.short_name,
      s.description_short, s.description_long,
      s.service_type_id, st.name AS service_type_name,
      s.parent_service_id, ps.name AS parent_service_name,
      s.base_price, s.list_price, s.currency, s.duration_minutes,
      s.wait_duration_min, s.promo_price, s.promo_valid_from, s.promo_valid_to,
      s.promo_label, s.online_bookable, s.is_active, s.is_combo,
      s.altegio_service_id, s.receipt_name, s.online_name, s.price_from, s.price_to
...
router.get("/available", async (_req: Request, res: Response) => {
  const sql = `
    SELECT s.id, s.name, s.short_name, s.duration_minutes, s.base_price, s.list_price
    FROM public.services s
    WHERE s.is_active = true AND s.online_bookable = true
    ORDER BY s.name;
  `;
  try {
...
router.post("/", async (req: Request, res: Response) => {
  const {
    ...
    description_short, description_long, promo_price, promo_valid_from,
    promo_valid_to, promo_label, online_bookable = true, is_active = true,
    is_combo = false,
  } = req.body || {};

  if (!name || !duration_minutes) {
    return res.status(400).json({ error: "A név és az időtartam (perc) kötelező." });
  }
  ...
  const sql = `
    INSERT INTO public.services (
      ...
      base_price, list_price, currency, duration_minutes, wait_duration_min,
      description_short, description_long, promo_price, promo_valid_from,
      promo_valid_to, promo_label, online_bookable, is_active, is_combo
    ) VALUES (
      $1,$2,$3,$4::uuid,$5::uuid,$6,COALESCE($7,$6),COALESCE($8,'HUF'),$9,$10,
      $11,$12,$13,$14::date,$15::date,$16,$17,$18,$19
    ) RETURNING *;
  `;
  const params = [
    name, code || null, short_name || null, service_type_id || null,
    parent_service_id || null, base_price ?? null, list_price ?? null,
    currency || "HUF", duration_minutes, wait_duration_min ?? null,
    description_short || null, description_long || null, promo_price ?? null,
    promo_valid_from || null, promo_valid_to || null, promo_label || null,
...
router.patch("/:id", async (req: Request, res: Response) => {
  ...
  const sql = `
    UPDATE public.services s SET
      name=COALESCE($2,s.name), code=$3, short_name=$4,
      service_type_id=$5::uuid, parent_service_id=$6::uuid,
      base_price=$7, list_price=$8, currency=COALESCE($9,s.currency),
      duration_minutes=COALESCE($10,s.duration_minutes), wait_duration_min=$11,
      description_short=$12, description_long=$13, promo_price=$14,
      promo_valid_from=$15::date, promo_valid_to=$16::date, promo_label=$17,
      online_bookable=COALESCE($18,s.online_bookable),
      is_active=COALESCE($19,s.is_active), is_combo=COALESCE($20,s.is_combo),
      updated_at=now()
    WHERE s.id=$1::uuid RETURNING *;
  `;
  const params = [
    id, name ?? null, code ?? null, short_name ?? null, service_type_id || null,
    parent_service_id || null, base_price ?? null, list_price ?? null,
    currency || null, duration_minutes ?? null, wait_duration_min ?? null,
    description_short ?? null, description_long ?? null, promo_price ?? null,
    promo_valid_from || null, promo_valid_to || null, promo_label ?? null,
    online_bookable, is_active, is_combo,
  ];
...
router.post("/reprice", async (req: Request, res: Response) => {
  ...
  try {
    ...
    await client.query(`
      UPDATE public.services s SET
        list_price=CASE WHEN list_price IS NULL THEN NULL ELSE ROUND(list_price*$1/$2)*$2 END,
        updated_at=now()
      WHERE s.is_active=true AND ($3::uuid IS NULL OR s.service_type_id=$3::uuid);
    `, [factor, roundTo, service_type_id || null]);

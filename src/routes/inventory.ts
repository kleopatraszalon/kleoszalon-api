  ...
  try {
    ...
    const { rows } = await db.query(
      `UPDATE product_stock_balances
       ...
       WHERE id=$1
       RETURNING id, product_id, location_id, quantity, min_quantity, unit_cost,
                 (quantity*unit_cost)::numeric AS stock_value, updated_at`,
...
router.get("/movements", async (req, res, next) => {
  try {
    ...
    if (req.query.location_id !== undefined) {
      ...
      else { params.push(locationId); filters.push(`m.location_id::text = $${params.length}::text`); }
    }
...
router.post("/movements", async (req, res, next) => {
  ...
  try {
    ...
    if (balanceResult.rows[0]) {
      await client.query(
        `UPDATE product_stock_balances SET quantity=$2, unit_cost=$3, min_quantity=$4, updated_at=now() WHERE id=$1`,
        [balanceResult.rows[0].id, newBalance, newUnitCost, newMinQuantity]
      );
    } else {
      await client.query(
        `INSERT INTO product_stock_balances (product_id, location_id, quantity, unit_cost, min_quantity, updated_at)
         VALUES ($1,$2,$3,$4,$5,now())`,
        [productId, locationId, newBalance, newUnitCost, newMinQuantity]
      );
    }

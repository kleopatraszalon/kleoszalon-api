...
router.get("/customer-360",async(req:AuthRequest,res:Response)=>{try{const s=await scope(req,res);if(!s)return;const rows=(await pool.query(`
 WITH a AS (
  ...
  FROM appointments WHERE tenant_id=$1::uuid AND ($2::uuid IS NULL OR location_id=$2::uuid) GROUP BY client_id
 ),v AS (
  ...
  FROM work_orders w WHERE w.tenant_id=$1::uuid AND ($2::uuid IS NULL OR w.location_id=$2::uuid) AND lower(COALESCE(w.status,''))='completed' GROUP BY client_id
 )
...
router.get("/inventory-intelligence",async(req:AuthRequest,res:Response)=>{try{
 ...
 const rows=(await pool.query(`WITH expired AS (
   ...
   WHERE w.location_id=ANY($1::text[])
  ...
  FROM stock s JOIN locations l ON l.id::text=s.location_id

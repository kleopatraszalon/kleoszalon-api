...
async function ensureTaxonomyTables(client: PoolClient) {
  // product_groups
  await client.query(`
  ...
  await client.query(
    `ALTER TABLE products ADD COLUMN IF NOT EXISTS web_description_hu text`
  );
  await client.query(
    `ALTER TABLE products ADD COLUMN IF NOT EXISTS web_description_en text`
  );
  await client.query(
    `ALTER TABLE products ADD COLUMN IF NOT EXISTS web_description_ru text`
  );
}

async function upsertDefaultGroup(client: PoolClient): Promise<{ id: string }> {
  // Első csoport, ha van
  const existing = await client.query(
    `SELECT id FROM product_groups ORDER BY created_at ASC NULLS LAST, id ASC LIMIT 1`
  );
  if (existing.rowCount) return { id: existing.rows[0].id };

  const id = randomUUID();
  await client.query(
    `INSERT INTO product_groups (id, name_hu, name_en, name_ru) VALUES ($1,$2,$3,$4)`,
    [id, "Egyéb", "Other", "Другое"]
  );
  return { id };
}
...
async function ensureCategoryId(
  ...
  if (c) {
    ...
    if (ok.rowCount) return c;

    // ha UUID-t kaptunk, de nincs ilyen: létrehozzuk a csoport alá
    await client.query(
      `INSERT INTO product_categories (id, product_group_id, name_hu, name_en, name_ru)
       VALUES ($1,$2,$3,$4,$5)
       ON CONFLICT (id) DO NOTHING`,
      [c, groupId, "Új kategória", "New category", "Новая категория"]
    );
    return c;
  }
  ...
  const existing = await client.query(
    `SELECT id FROM product_categories WHERE product_group_id=$1 ORDER BY created_at ASC NULLS LAST, id ASC LIMIT 1`,
    [groupId]
  );
...
groupPaths.forEach((p) => {
  router.get(p, async (_req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        await ensureTaxonomyTables(client);
        const r = await client.query(
          `SELECT id, name_hu, name_en, name_ru FROM product_groups ORDER BY created_at ASC NULLS LAST, id ASC`
        );
        await client.query("COMMIT");
  ...
  router.post(p, async (req: Request, res: Response) => {
    try {
      ...
      const gid = (id ? String(id).trim() : randomUUID()) as string;
      ...
      try {
        ...
        await client.query(
          `INSERT INTO product_groups (id, name_hu, name_en, name_ru)
           VALUES ($1,$2,$3,$4)
        ...
        const out = await client.query(
          `SELECT id, name_hu, name_en, name_ru FROM product_groups WHERE id=$1`,
          [gid]
        );
        await client.query("COMMIT");
...
categoryPaths.forEach((p) => {
  router.get(p, async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        const r = groupId
          ? await client.query(
              `SELECT id, product_group_id, name_hu, name_en, name_ru
  ...
  router.post(p, async (req: Request, res: Response) => {
    try {
      const { id, product_group_id, group_id, name_hu, name_en, name_ru } =
        req.body || {};
      ...
      try {
        ...
        const cid = (id ? String(id).trim() : randomUUID()) as string;
        ...
        const out = await client.query(
          `SELECT id, product_group_id, name_hu, name_en, name_ru FROM product_categories WHERE id=$1`,
          [cid]
        );
        await client.query("COMMIT");
        return res.status(201).json(out.rows[0]);
...
router.post("/products", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const retailPrice =
      body.retail_price_gross !== undefined ? toNumber(body.retail_price_gross) : null;
    ...
    const locationId =
      ...
      ((req as any).user?.location_id ? String((req as any).user.location_id) : null);
...
router.put("/products/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    let groupId: string = current.product_group_id;
    if (explicitGroup !== undefined) {
      groupId = await ensureGroupId(client, explicitGroup);
    }

    let categoryId: string | null = current.product_category_id ?? null;
    if (await hasColumn(client, "products", "product_category_id")) {
    ...
    // többnyelvű mezők
    if (await hasColumn(client, "products", "name_hu")) push("name_hu", name_hu ?? null);
    if (await hasColumn(client, "products", "name_en")) push("name_en", name_en ?? null);
    if (await hasColumn(client, "products", "name_ru")) push("name_ru", name_ru ?? null);

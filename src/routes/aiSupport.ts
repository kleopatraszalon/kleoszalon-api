...
function cleanMessages(value: unknown): ChatMessage[] {
  ...
  return value.filter((m: any) => m && (m.role === "user" || m.role === "assistant") && typeof m.content === "string")
    ...
    .filter((m: ChatMessage) => m.content.length > 0).slice(-10);
}
  ...
  const { rows } = await db.query(
    `SELECT COUNT(*)::int AS request_count,
     ...
     WHERE created_at >= date_trunc('month', now())
       AND ($1 = '' OR user_key = $1)`, [key]);
  const r = rows[0] || {};
  return { request_count:Number(r.request_count||0), input_tokens:Number(r.input_tokens||0), output_tokens:Number(r.output_tokens||0), estimated_cost_usd:Number(r.estimated_cost_usd||0) };
}
...
router.get("/stats", requireAuth, requireFeature("ai_stats"), async (req: AuthRequest, res, next) => {
  try {
    const key = userKey(req);
    const [globalUsage, myUsage, daily] = await Promise.all([
      monthlyUsage(""), monthlyUsage(key),
      db.query(`SELECT created_at::date AS day, COUNT(*)::int AS requests, COALESCE(SUM(estimated_cost_usd),0)::numeric AS estimated_cost_usd
                FROM ai_usage_log WHERE created_at >= CURRENT_DATE - INTERVAL '29 days'
...
router.post("/chat", requireAuth, requireFeature("ai_use"), async (req: AuthRequest, res) => {
  const key = userKey(req);
  if (!allowRequest(key)) return res.status(429).json({ code:"local_rate_limit", message:"Túl sok AI-kérés. Kérlek próbáld újra egy perc múlva." });

  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) return res.status(503).json({ code:"missing_api_key", message:"Az AI támogatás még nincs aktiválva. A szerveren be kell állítani az OPENAI_API_KEY környezeti változót." });

  const messages = cleanMessages(req.body?.messages);
  if (!messages.length || messages[messages.length - 1].role !== "user") return res.status(400).json({ code:"missing_user_message", message:"Hiányzik a felhasználói kérdés." });

  try {
    const cfg = limits();
    const [globalUsage, myUsage] = await Promise.all([monthlyUsage(""), monthlyUsage(key)]);
    if (cfg.monthlyBudgetUsd > 0 && globalUsage.estimated_cost_usd >= cfg.monthlyBudgetUsd) return res.status(429).json({ code:"monthly_budget_exhausted", message:"A VIR havi AI-költségkerete elfogyott.", usage:globalUsage, limit:cfg.monthlyBudgetUsd });
    if (cfg.userMonthlyBudgetUsd > 0 && myUsage.estimated_cost_usd >= cfg.userMonthlyBudgetUsd) return res.status(429).json({ code:"user_budget_exhausted", message:"Elérted a havi személyes AI-költségkeretedet.", usage:myUsage, limit:cfg.userMonthlyBudgetUsd });
    if (cfg.userMonthlyRequestLimit > 0 && myUsage.request_count >= cfg.userMonthlyRequestLimit) return res.status(429).json({ code:"user_request_limit", message:"Elérted a havi AI-kérésszám limitet.", usage:myUsage, limit:cfg.userMonthlyRequestLimit });

    const page=String(req.body?.context?.pathname||"").slice(0,300), pageTitle=String(req.body?.context?.page_title||"").slice(0,300), locationName=String(req.body?.context?.location_name||"").slice(0,300), role=String(req.body?.context?.role||"").slice(0,100);
    const contextText=[page?`Aktuális útvonal: ${page}`:"",pageTitle?`Aktuális oldal: ${pageTitle}`:"",locationName?`Kiválasztott telephely: ${locationName}`:"",role?`Felhasználói szerepkör: ${role}`:""].filter(Boolean).join("\n");
    const input=[...(contextText?[{role:"developer",content:[{type:"input_text",text:contextText}]}]:[]),...messages.map(m=>({role:m.role,content:[{type:m.role==="assistant"?"output_text":"input_text",text:m.content}]}))];

    const response = await axios.post("https://api.openai.com/v1/responses", { model:process.env.OPENAI_MODEL||"gpt-5-mini", instructions:systemInstructions, input, store:false, max_output_tokens:700 }, { headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"}, timeout:30_000 });
    const data:any=response.data;
    let answer=String(data?.output_text||"").trim();
    if(!answer&&Array.isArray(data?.output)) answer=data.output.flatMap((item:any)=>Array.isArray(item?.content)?item.content:[]).filter((item:any)=>item?.type==="output_text"&&typeof item?.text==="string").map((item:any)=>item.text).join("\n").trim();
    ...
    await db.query(`INSERT INTO ai_usage_log (user_key, model, input_tokens, output_tokens, estimated_cost_usd) VALUES ($1,$2,$3,$4,$5)`,[key,data?.model||process.env.OPENAI_MODEL||"gpt-5-mini",inputTokens,outputTokens,estimatedCost]);
    ...
    const status=Number(err?.response?.status||0), openAiError=err?.response?.data?.error||{}, openAiCode=String(openAiError?.code||openAiError?.type||"openai_error"), detail=String(openAiError?.message||err?.message||"Ismeretlen AI hiba").slice(0,600);

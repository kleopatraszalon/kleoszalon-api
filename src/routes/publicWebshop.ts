...
import pool from "../db"; // ha nálad export { pool }, akkor írd át
  ...
  try {
    ...
    if (existing.rows.length > 0) {
      return res.status(200).json({
        message:
          "Már létezik felhasználó ezzel az e-mail címmel. Jelentkezz be a meglévő fiókoddal.",
      });
    }
...
router.post("/validate-coupon", async (req: Request, res: Response) => {
  try {
    ...
    if (totalGross < minOrder) {
      return res.json({
        valid: false,
        message: `A kupon minimum rendeléshez kötött (${minOrder.toLocaleString(
          "hu-HU"
        )} Ft).`,
      });
    }
  ...
  try {
    ...
    if (coupon && coupon.code) {
      ...
      const couponRes = await client.query(
        `SELECT id, usage_limit, used_count FROM coupons WHERE code = $1 LIMIT 1`,
        [couponCode]
      );
      ...
      if (couponRes.rows.length > 0) {
        ...
        if (
          couponRes.rows[0].usage_limit == null ||

...
router.get("/dashboard", async (req: AuthRequest, res: Response) => { try {
  const query = (req.query || {}) as VirQueryParams; const today = new Date(); const defaultTo = today.toISOString().slice(0, 10); const monthStart = new Date(today.getFullYear(), today.getMonth(), 1).toISOString().slice(0, 10);
  const from = parseDateInput(query.from, monthStart); const to = parseDateInput(query.to, defaultTo); const locationId = getScopedLocationId(req, res); if (locationId === undefined) return;

...
router.use("/p1", virP1Router);router.use("/p2", virP2Router);router.use("/p3/revenue-leakage", virRevenueLeakageRouter);router.use("/p3", virP3Router);router.use("/p4", virP4Router);router.use("/p5", virP5Router);router.use("/p6", virP6Router);router.use("/p6", virP6BindingRouter);router.use("/p7", virP7Router);router.use("/p8", virP8Router);router.use("/p9", virP9Router);router.use("/p10", virP10Router);router.use("/p11", virP11Router);router.use("/p12", virP12Router);router.use("/p13", virP13Router);router.use("/p14", virP14Router);router.use("/p15", virP15Router);router.use("/p16", virP16Router);
router.use("/reception", virReceptionGuestActionsRouter);router.use("/migration-center", migrationCenterRouter);router.use("/receipt-compliance", receiptComplianceRouter);router.use("/device-control", deviceBridgeResultRouter);router.use("/device-control", deviceControlRouter);router.use("/fitness/lockers", fitnessLockerRouter);router.use("/fitness", fitnessRouter);
...
router.get("/dashboard",async(req:AuthRequest,res:Response)=>{try{const query=(req.query||{}) as VirQueryParams;const today=new Date();const defaultTo=today.toISOString().slice(0,10);const monthStart=new Date(today.getFullYear(),today.getMonth(),1).toISOString().slice(0,10);const from=parseDateInput(query.from,monthStart),to=parseDateInput(query.to,defaultTo),locationId=getScopedLocationId(req,res);if(locationId===undefined)return;const{rows}=await pool.query(`SELECT * FROM public.vir_dashboard_summary($1::date, $2::date, $3::uuid)`,[from,to,locationId]);return res.json({ok:true,summary:rows[0]||{revenue_total:0,paid_total:0,appointments_count:0,completed_count:0,cancelled_count:0,no_show_count:0,avg_basket:0,cancellation_rate_percent:0,no_show_rate_percent:0}})}catch(error){return res.status(500).json({ok:false,error:toErrorMessage(error)})}});
router.get("/revenue-series",async(req:AuthRequest,res:Response)=>{try{const query=(req.query||{}) as VirQueryParams;const today=new Date();const defaultTo=today.toISOString().slice(0,10);const monthStart=new Date(today.getFullYear(),today.getMonth(),1).toISOString().slice(0,10);const from=parseDateInput(query.from,monthStart),to=parseDateInput(query.to,defaultTo),locationId=getScopedLocationId(req,res);if(locationId===undefined)return;const{rows}=await pool.query(`SELECT * FROM public.vir_revenue_series($1::date, $2::date, $3::uuid) ORDER BY day`,[from,to,locationId]);return res.json({ok:true,rows})}catch(error){return res.status(500).json({ok:false,error:toErrorMessage(error)})}});

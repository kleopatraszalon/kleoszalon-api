...
router.get(["/", "/summary"], async (req: AuthRequest, res, next) => {
  try {
    ...
    const roles = Array.isArray(raw) ? raw.map(String) : String(raw||"").replace(/[\[\]"]/g,"").split(",").map(x=>x.trim()).filter(Boolean);
    ...
    const revenueRes = await safeQuery(`WITH closed AS (
      ...
      COALESCE(SUM(tip_amount),0)::numeric tips,COUNT(*)::int closed_workorders FROM closed`, params, emptyRevenue, "pénzügy", warnings);

    let stockRes:any;
    try {
      stockRes = await db.query(`SELECT COALESCE(SUM(b.quantity*COALESCE(b.unit_cost,0)),0)::numeric inventory_value,
    ...
    const crmRes = await safeQuery(`SELECT COUNT(*)::int visits,COUNT(DISTINCT vh.profile_id)::int unique_guests,
      COALESCE(SUM(vh.amount_paid),0)::numeric guest_revenue,COALESCE(AVG(vh.amount_paid),0)::numeric avg_guest_spend
    ...
    const staffRes = await safeQuery(`SELECT wo.employee_id::text employee_id,COALESCE(e.full_name,'Nincs munkatárs') employee_name,
      COUNT(DISTINCT wo.id)::int workorder_count,COALESCE(SUM(wi.line_total),0)::numeric revenue,
      COALESCE(SUM(wi.line_total)/NULLIF(COUNT(DISTINCT wo.id),0),0)::numeric avg_ticket
      FROM work_orders wo LEFT JOIN employees e ON e.id=wo.employee_id LEFT JOIN work_order_items wi ON wi.work_order_id=wo.id
      WHERE wo.financial_closed_at::date BETWEEN $1::date AND $2::date AND ${loc}
      GROUP BY wo.employee_id,e.full_name ORDER BY revenue DESC LIMIT 20`,params,{},"munkatársi pénzügy",warnings);

    const r = revenueRes.rows[0] || emptyRevenue, gross=n(r.gross_revenue), service=n(r.service_revenue), product=n(r.product_revenue);
    res.json({ period:{from,to}, location_id:locationId,

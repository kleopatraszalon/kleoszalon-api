...
async function ensureRuntimeSchema(c:any){if(runtimeReady)return;await c.query(`
 ...
 CREATE TABLE IF NOT EXISTS product_stock_balances(
  id bigserial PRIMARY KEY,product_id uuid NOT NULL REFERENCES products(id) ON DELETE CASCADE,location_id uuid,quantity numeric(14,3) NOT NULL DEFAULT 0,updated_at timestamptz NOT NULL DEFAULT now());
 ...
 CREATE TABLE IF NOT EXISTS inventory_movements(
  ...
  movement_type varchar(32) NOT NULL,quantity numeric(14,3) NOT NULL,balance_after numeric(14,3),note text,created_by bigint,created_at timestamptz NOT NULL DEFAULT now());
...
async function consumeStock(c:any,workOrder:any){if(workOrder.stock_consumed_at)return;const items=(await c.query(`SELECT product_id,SUM(quantity)::numeric quantity FROM work_order_items WHERE work_order_id=$1 AND item_type='product' AND product_id IS NOT NULL GROUP BY product_id`,[workOrder.id])).rows;for(const item of items){const qty=Number(item.quantity||0);if(!(qty>0))continue;await c.query(`INSERT INTO product_stock_balances(product_id,location_id,quantity) SELECT $1,$2::uuid,0 WHERE NOT EXISTS(SELECT 1 FROM product_stock_balances WHERE product_id=$1 AND location_id IS NOT DISTINCT FROM $2::uuid)`,[item.product_id,workOrder.location_id||null]);const b=await c.query(`UPDATE product_stock_balances SET quantity=quantity-$3,updated_at=now() WHERE product_id=$1 AND location_id IS NOT DISTINCT FROM $2::uuid AND quantity>=$3 RETURNING quantity`,[item.product_id,workOrder.location_id||null,qty]);if(!b.rows[0]){const p=await c.query(`SELECT name FROM products WHERE id=$1`,[item.product_id]);throw new Error(`Nincs elegendő készlet: ${p.rows[0]?.name||item.product_id}`)}await c.query(`INSERT INTO inventory_movements(product_id,location_id,work_order_id,movement_type,quantity,balance_after,note) VALUES($1,$2::uuid,$3,'work_order_consumption',$4,$5,$6) ON CONFLICT DO NOTHING`,[item.product_id,workOrder.location_id||null,workOrder.id,-qty,b.rows[0].quantity,'Automatikus készletcsökkentés végleges munkalap-záráskor']);}}
...
router.post('/workorders/:id/finalize',async(req:AuthRequest,res,next)=>{const c=await db.connect();try{await ensureRuntimeSchema(c);await c.query('BEGIN');const wo=(await c.query(`SELECT * FROM work_orders WHERE id=$1 FOR UPDATE`,[req.params.id])).rows[0];if(!wo){await c.query('ROLLBACK');return res.status(404).json({message:'A munkalap nem található.'})}if(wo.locked_at){await c.query('COMMIT');return res.json({idempotent:true,work_order:wo,message:'A munkalap már lezárt és archivált.'})}if(String(wo.status||'')!=='in_progress')throw new Error('A munkalap csak Folyamatban állapotból zárható véglegesen.');if(String(wo.payment_status||'')!=='paid'||!wo.financial_closed_at)throw new Error('A munkalap csak teljesen kifizetett és pénzügyileg lezárt állapotban véglegesíthető.');
 ...
 const hasHistory=(await c.query(`SELECT to_regclass('public.work_order_status_history') IS NOT NULL ok`)).rows[0]?.ok;if(hasHistory){await c.query(`INSERT INTO work_order_status_history(work_order_id,status_kind,from_status,to_status,changed_by,reason,note,metadata) VALUES($1,'document',$2,'completed',$3,'FINALIZATION',$4,$5::jsonb)`,[wo.id,previousDocumentStatus,actor(req),'Végleges lezárás: fizetés, készlet, pénzügy, jutalék és archiválás lezárva.',JSON.stringify({service_status_from:wo.status,service_status_to:'completed'})])}
 const invoice=await ensureInvoiceDraft(c,{...wo,...updated},actor(req));if(settlement)await c.query(`UPDATE loyalty_checkout_settlements SET finalized_at=COALESCE(finalized_at,now()),finalization_payload=$2::jsonb WHERE work_order_id=$1`,[wo.id,JSON.stringify({financial_account_id:defaultAccount||null,payment_accounts:mappings,finalized_by:actor(req),invoice_id:invoice.id,invoice_no:invoice.invoice_no})]);await c.query('COMMIT');const archive=(await db.query(`SELECT work_order_number,archived_at,terminal_status,snapshot_hash FROM work_order_archive WHERE work_order_id=$1`,[wo.id])).rows[0]||null;res.json({work_order:updated,archive,invoice,finalized:true,appointment_completed:Boolean(wo.appointment_id),loyalty_settlement:Boolean(settlement)});
 }catch(e:any){await c.query('ROLLBACK').catch(()=>undefined);const m=String(e?.message||'A munkalap végleges lezárása nem sikerült.');if(/Folyamatban|pénztári|kifizetett|pénzügyi|készlet|számla|fizetéshez/i.test(m))return res.status(409).json({message:m});next(e)}finally{c.release()}});

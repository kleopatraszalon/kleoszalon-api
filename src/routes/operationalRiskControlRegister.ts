...
router.post('/sync',async(_req:AuthRequest,res,next)=>{try{res.json(await syncOperationalRiskRegister())}catch(error){next(error)}});
router.post('/governance-cycle',async(_req:AuthRequest,res,next)=>{try{res.json(await runOperationalRiskGovernanceCycle())}catch(error){next(error)}});

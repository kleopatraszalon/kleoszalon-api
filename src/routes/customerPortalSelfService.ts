...
router.patch("/self-service/appointments/:id/reschedule",asyncRoute(async(req,res)=>{
  ...
  try{
    ...
    const local=(await cx.query(`SELECT ($1::timestamptz AT TIME ZONE 'Europe/Budapest')::date day,EXTRACT(HOUR FROM ($1::timestamptz AT TIME ZONE 'Europe/Budapest'))::int*60+EXTRACT(MINUTE FROM ($1::timestamptz AT TIME ZONE 'Europe/Budapest'))::int start_min,($2::timestamptz AT TIME ZONE 'Europe/Budapest')::date end_day,EXTRACT(HOUR FROM ($2::timestamptz AT TIME ZONE 'Europe/Budapest'))::int*60+EXTRACT(MINUTE FROM ($2::timestamptz AT TIME ZONE 'Europe/Budapest'))::int end_min`,[start.toISOString(),end.toISOString()])).rows[0];
    if(String(local.day)!==String(local.end_day)||Number(local.start_min)<Number(cfg.opening_minute||480)||Number(local.end_min)>Number(cfg.closing_minute||1200)){await cx.query("ROLLBACK");return res.status(409).json({error:"Az új időpont a szalon online foglalási nyitvatartásán kívül esik."});}

...
router.get('/dashboard/summary',async(req:AuthRequest,res,next)=>{
  ...
  try{
    const summary=(await db.query(`SELECT COUNT(*)::int total,COUNT(*) FILTER(WHERE NULLIF(to_jsonb(w)->>'archived_at','') IS NOT NULL OR NULLIF(to_jsonb(w)->>'locked_at','') IS NOT NULL)::int archived FROM work_orders w WHERE ${archivedPredicate}`)).rows[0]||{};
    const recent=(await db.query(`SELECT w.id::text id,COALESCE(NULLIF(to_jsonb(w)->>'work_order_number',''),w.id::text) work_order_number,COALESCE(NULLIF(to_jsonb(w)->>'status',''),'completed') status,NULLIF(to_jsonb(w)->>'created_at','') created_at,NULLIF(to_jsonb(w)->>'locked_at','') locked_at,NULLIF(to_jsonb(w)->>'archived_at','') archived_at,false can_edit FROM work_orders w WHERE ${archivedPredicate} ORDER BY COALESCE(NULLIF(to_jsonb(w)->>'archived_at',''),NULLIF(to_jsonb(w)->>'locked_at',''),NULLIF(to_jsonb(w)->>'created_at','')) DESC NULLS LAST LIMIT 8`)).rows;
...
router.post('/:id/settle-recovery',async(req:AuthRequest,res,next)=>{
 try{
  if(!canEditRole(req.user?.role))return res.status(403).json({message:'A pénzügyi lezárást csak adminisztrátor, recepciós vagy üzletvezető végezheti.'});
  if(!isAdmin(req.user?.role)){
   const locationId=String(req.user?.location_id||'');
   const allowed=(await db.query(`SELECT 1 FROM work_orders WHERE id::text=$1 AND location_id::text=$2 LIMIT 1`,[req.params.id,locationId])).rows[0];
   if(!allowed)return res.status(404).json({message:'A munkalap nem ehhez a szalonhoz tartozik.'});
  }
  const result=await settleWorkOrderWithoutShift(String(req.params.id),req.body||{},req.user?.email||String(req.user?.id||''));
...
router.patch('/:id/lifecycle',async(req:AuthRequest,res,next)=>{
  ...
  try{
    ...
    if(requested==='arrived'||requested==='in_progress'){
      try{lateness=await recordWorkOrderArrival(String(req.params.id))}catch(error:any){console.warn('[workorders-lifecycle-hotfix] lateness tracking skipped',error?.code||'',error?.message||error)}
    }

...
router.get('/options',async(req:AuthRequest,res:Response,next:NextFunction)=>{
  try{
    ...
    const [locationQ,employees,clients,services,products,locations]=await Promise.all([
      ...
      optionalRows('clients',db.query(`SELECT c.id::text,COALESCE(NULLIF(to_jsonb(c)->>'full_name',''),NULLIF(to_jsonb(c)->>'name',''),'') name,to_jsonb(c)->>'phone' phone,to_jsonb(c)->>'email' email,NULLIF(to_jsonb(c)->>'location_id','') location_id FROM clients c WHERE (NULLIF(to_jsonb(c)->>'location_id','')=$1 OR NULLIF(to_jsonb(c)->>'location_id','') IS NULL) AND ($2::text IS NULL OR NULLIF(to_jsonb(c)->>'tenant_id','') IS NULL OR NULLIF(to_jsonb(c)->>'tenant_id','')=$2) AND COALESCE(NULLIF(to_jsonb(c)->>'is_active','')::boolean,true)=true ORDER BY 2 LIMIT 500`,[locationId,tenantId])),

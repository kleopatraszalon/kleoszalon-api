...
async function tenantCanAccess(req:AuthRequest,locationId:string,workOrderTenantId=''){
 const tokenTenant=String(req.user?.tenant_id||'').trim(),woTenant=String(workOrderTenantId||'').trim();
 if(tokenTenant&&woTenant&&tokenTenant===woTenant)return true;
 if(tokenTenant&&locationId){try{if(await locationBelongsToTenant(locationId,tokenTenant))return true}catch{}}
 try{
  const tenant=await resolveTenantIdentity(req);if(!tenant)return false;
  const resolved=String(tenant.id||'').trim();
  if(resolved&&woTenant&&resolved===woTenant)return true;
  return Boolean(locationId&&resolved&&await locationBelongsToTenant(locationId,resolved));
 }catch{return false}
}
...
async function canAccessWorkOrder(req:AuthRequest,locationId:string,employeeId:string,workOrderTenantId=''){
 if(global(req))return true;
 const own=String(req.user?.location_id||'').trim();
 if(own&&own===locationId)return true;
 if(await tenantCanAccess(req,locationId,workOrderTenantId))return true;
...
router.post('/pending-selection',async(req:AuthRequest,res:Response)=>{try{
 ...
 if(!global(req)){const locations=Array.isArray(entity.locations)?entity.locations:[];if(!locations.length)return res.status(403).json({message:'A kiválasztott cég nincs hozzárendelve a saját szalonhoz.'});const tenantMatches=await Promise.all(locations.map((x:any)=>tenantCanAccess(req,String(x.id||''))));if(!tenantMatches.some(Boolean))return res.status(403).json({message:'A kiválasztott cég nincs hozzárendelve a saját szalonhoz.'})}

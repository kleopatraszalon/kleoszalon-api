...
router.post('/pending-selection',async(req:AuthRequest,res:Response)=>{try{
 await ensureWorkOrderOperationalFinance();if(!chooser(req))return res.status(403).json({message:'Kibocsátó céget csak adminisztrátor, vezető vagy recepciós választhat.'});

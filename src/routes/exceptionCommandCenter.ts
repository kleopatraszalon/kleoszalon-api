...
router.post("/sync",async(_req:AuthRequest,res,next)=>{try{const sync=await syncExceptionCommandCenter();const consistency=await reconcileExceptionCommandCenterConsistency();const intelligence=await runExceptionIntelligenceCycle();res.json({...sync,consistency,intelligence})}catch(error){next(error)}});
...
router.post("/intelligence/run",async(_req:AuthRequest,res,next)=>{try{res.json(await runExceptionIntelligenceCycle())}catch(error){next(error)}});

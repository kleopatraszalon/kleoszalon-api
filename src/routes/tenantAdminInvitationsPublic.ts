...
router.post("/:token/accept", async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(201).json({ ok:true, tenant_id:result.tenant_id, tenant_name:result.tenant_name, email:result.email, message:"Az adminisztrátori fiók aktiválása sikerült. Most már bejelentkezhet." });
  } catch (error:any) {

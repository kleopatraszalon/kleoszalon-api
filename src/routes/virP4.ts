...
async function scope(req: AuthRequest, res: Response): Promise<Scope | undefined> {
  // A korábbi böngésző-session még hordozhat legacy UUID tenant_id-t, miközben
  // az aktuális SaaS tenant kulcs BIGINT. Mindig a központi tenant rétegen oldjuk fel.
...
router.get("/workforce-optimizer", async (req: AuthRequest, res: Response) => {
  try {
    ...
    for (const loc of locations) {
      const rows = (
        await pool.query(
          `WITH dates AS (
             SELECT d::date day
             FROM generate_series(current_date,current_date+($2::int-1),interval '1 day') d
           ), hist AS (
             ...
             FROM appointments a
             WHERE a.tenant_id::text=$3::text AND a.location_id=$1::uuid
             ...
             WHERE ws.work_date>=current_date AND ws.work_date<current_date+$2::int
               AND ws.status='published'
               AND e.tenant_id::text=$3::text AND e.location_id=$1::uuid AND COALESCE(e.active,true)=true
             GROUP BY 1
           )
           ...
           FROM dates d
           LEFT JOIN hist h ON h.dow=EXTRACT(ISODOW FROM d.day)::int
           LEFT JOIN booked b ON b.day=d.day
           LEFT JOIN shifts sh ON sh.day=d.day
           ORDER BY d.day`,
          [loc.id, days, s.tenantId]
        )
      ).rows;

      const skills = (
        await pool.query(
          `SELECT COUNT(DISTINCT o.service_id)::int configured_services,
           ...
           WHERE e.tenant_id::text=$2::text AND e.location_id=$1::uuid AND COALESCE(e.active,true)=true`,
          [loc.id, s.tenantId]
        )
      ).rows[0] || {};
      ...
      for (const r of rows) {
        ...
        items.push({
          ...
          day: r.day,
          booked_count: num(r.booked_count),
          historical_daily_bookings: Math.round(num(r.historical_daily_bookings) * 10) / 10,
          demand_minutes: Math.round(demandMinutes),
          scheduled_staff: scheduled,
          scheduled_minutes: Math.round(num(r.scheduled_minutes)),
          required_staff: requiredStaff,
          staff_gap: gap,
          utilization_percent: utilization,
          status,
          skill_coverage: {
            configured_services: num(skills.configured_services),
            blocked_skill_rows: num(skills.blocked_skill_rows),
            expiring_30d: num(skills.expiring_30d),
          },
          rationale: gap > 0
            ? `A várható terheléshez ${gap} további munkatárs-kapacitás javasolt.`
            : gap < 0
              ? `A beosztott kapacitás ${Math.abs(gap)} fővel meghaladja a becsült igényt.`
              : "A beosztott létszám illeszkedik a becsült igényhez.",
        });
      }
    }
    ...
    return res.json({
      ok: true,
      model: "forecast_capacity_workforce_optimizer_v1",
      days,
      automatic_scheduling: false,
      decision_support_only: true,
      summary: {
        days_analyzed: items.length,
        shortage_days: shortages.length,
        critical_days: items.filter(x => x.status === "CRITICAL_SHORTAGE").length,
        total_staff_gap: shortages.reduce((a, x) => a + x.staff_gap, 0),
        expiring_qualifications: Math.max(0, ...items.map(x => x.skill_coverage.expiring_30d)),
      },
      items,
    });
  } catch (e: any) {
    return res.status(500).json({ ok: false, error: e?.message || "workforce_optimizer_failed" });
  }
});

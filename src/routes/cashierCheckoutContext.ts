...
router.get('/register-history',async(req:AuthRequest,res,next)=>{try{
 const locationId=String(req.query.location_id||req.user?.location_id||'').trim();const registerId=String(req.query.register_id||'').trim();const date=String(req.query.date||'').trim()||localDate();

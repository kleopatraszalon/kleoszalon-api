...
const dayStartUtc = (date: string) => new Date(`${date}T00:00:00`);
...
router.get("/catalog", async (req, res) => {
  try {
    ...
    const [serviceRows, employeeRows, cfg] = await Promise.all([
      db.query(`SELECT id,name,COALESCE(duration_minutes,30)::int duration_minutes,
        COALESCE(promo_price,list_price,base_price,0)::numeric price,
        COALESCE(service_type_name,category_name,'Egyéb szolgáltatások') category_name
        FROM services WHERE is_active=true ORDER BY COALESCE(service_type_name,category_name,''),name`),
      db.query(`SELECT id,COALESCE(full_name,name,'Munkatárs') full_name,photo_url,color
        FROM employees WHERE active=true AND (location_id=$1::uuid OR location_id IS NULL)
        ORDER BY COALESCE(full_name,name,'')`, [locationId]),
      settings(locationId),
    ]);
...
router.get("/availability", async (req, res) => {
  try {
    ...
    const serviceResult = await db.query(`SELECT id,COALESCE(duration_minutes,30)::int duration_minutes FROM services WHERE id=ANY($1::uuid[]) AND is_active=true`, [serviceIds]);
    ...
    const employees = await db.query(`SELECT id,COALESCE(full_name,name,'Munkatárs') full_name FROM employees
      WHERE active=true AND (location_id=$1::uuid OR location_id IS NULL) AND ($2::uuid IS NULL OR id=$2::uuid)
      ORDER BY COALESCE(full_name,name,'')`, [locationId, employeeId || null]);
...
router.post("/book", async (req, res) => {
  ...
  try {
    ...
    const services = await cx.query(`SELECT id,name,COALESCE(duration_minutes,30)::int duration_minutes,
      COALESCE(promo_price,list_price,base_price,0)::numeric price FROM services
      WHERE id=ANY($1::uuid[]) AND is_active=true`, [serviceIds]);
    ...
    const conflict = await cx.query(`SELECT id FROM appointments WHERE employee_id=$1::uuid
      AND status NOT IN ('cancelled','canceled','no_show') AND start_time<$3::timestamptz AND end_time>$2::timestamptz LIMIT 1`,
    ...
    let client = await cx.query(`SELECT id FROM clients WHERE location_id=$1::uuid AND
      (($2<>'' AND regexp_replace(COALESCE(phone,''),'[^0-9]','','g')=regexp_replace($2,'[^0-9]','','g')) OR ($3<>'' AND lower(COALESCE(email,''))=lower($3)))
      ORDER BY updated_at DESC NULLS LAST LIMIT 1`, [locationId, phone, email]);
    let clientId = client.rows[0]?.id;
    if (!clientId) {
      client = await cx.query(`INSERT INTO clients(full_name,name,phone,email,location_id,marketing_consent,is_active,source,created_at,updated_at)
        VALUES($1,$1,$2,$3,$4::uuid,$5,true,'online_booking',now(),now()) RETURNING id`,
        [fullName, phone || null, email || null, locationId, Boolean(req.body?.marketing_consent)]);
    ...
    const appointment = await cx.query(`INSERT INTO appointments
      (employee_id,client_id,location_id,title,start_time,end_time,status,notes,booking_source,cancellation_token,confirmation_required,confirmed_at,updated_at)
      VALUES($1::uuid,$2::uuid,$3::uuid,$4,$5::timestamptz,$6::timestamptz,$7,$8,'online',$9::uuid,$10,$11,now()) RETURNING id`,
      [employeeId, clientId, locationId, title, start.toISOString(), end.toISOString(), status, req.body?.note || "", token,
       Boolean(cfg.require_staff_confirmation), cfg.require_staff_confirmation ? null : new Date().toISOString()]);
    ...
    await cx.query(`INSERT INTO appointment_change_log(appointment_id,action,actor_key,after_data,note)
      VALUES($1::uuid,'online_created','public',$2::jsonb,$3)`, [appointment.rows[0].id, JSON.stringify({ status,start_time:start,end_time:end,employee_id:employeeId }), "Online foglalás"]);
    await cx.query("COMMIT");
    res.status(201).json({ id: appointment.rows[0].id, status, confirmation_required: Boolean(cfg.require_staff_confirmation), cancellation_token: token, online_discount_percent: Number(cfg.online_discount_percent || 0) });
  } catch (error: any) {
...
router.post("/waitlist", async (req, res) => {
  try {
    ...
    const { rows } = await db.query(`INSERT INTO booking_waitlist
      (location_id,client_name,phone,email,service_ids,preferred_employee_id,preferred_from,preferred_to,note,source)
      VALUES($1::uuid,$2,$3,$4,$5::uuid[],$6::uuid,$7::timestamptz,$8::timestamptz,$9,'online') RETURNING id,status,created_at`,
      [locationId,name,req.body?.phone||null,req.body?.email||null,serviceIds,req.body?.employee_id||null,req.body?.preferred_from||null,req.body?.preferred_to||null,req.body?.note||null]);
...
router.post("/cancel/:token", async (req, res) => {
  try {
    ...
    const { rows } = await db.query(`UPDATE appointments SET status='cancelled',cancellation_reason=$2,cancelled_at=now(),updated_at=now()
      WHERE cancellation_token=$1::uuid AND status NOT IN ('cancelled','canceled','completed','paid') RETURNING id`, [req.params.token, reason]);

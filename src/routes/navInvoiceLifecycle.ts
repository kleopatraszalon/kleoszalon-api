...
router.get('/dashboard',async(req:AuthRequest,res,next)=>{try{
  ...
  const stats=await db.query(`SELECT COUNT(*)::int total,COUNT(*) FILTER(WHERE nav_status='done')::int done,COUNT(*) FILTER(WHERE nav_status IN ('error','aborted'))::int failed,COUNT(*) FILTER(WHERE nav_status IN ('submitted','processing','prepared'))::int pending,COUNT(*) FILTER(WHERE nav_validation_status='failed')::int invalid,COUNT(*) FILTER(WHERE nav_xsd_validation_status='failed')::int xsd_invalid,COUNT(*) FILTER(WHERE nav_xsd_validation_status='engine_error')::int xsd_engine_error FROM finance_invoices WHERE ($1::text='' OR location_id::text=$1 OR location_id IS NULL)`,[locationId]);
  const invoices=await db.query(`SELECT id,invoice_no,partner_name,gross_total,issue_date,status,nav_status,nav_validation_status,nav_xsd_validation_status,nav_xsd_schema_revision,invoice_type,original_invoice_number,nav_transaction_id FROM finance_invoices WHERE ($1::text='' OR location_id::text=$1 OR location_id IS NULL) ORDER BY created_at DESC LIMIT 50`,[locationId]);

...
router.get('/plan',async(req:AuthRequest,res,next)=>{try{await ensureSchema();if(!canEdit(req))return res.status(403).json({message:'Nincs jogosultság a munkalap anyagtervéhez.'});const locationId=scopedLocation(req,String(req.query.location_id||''));if(!locationId)return res.status(400).json({message:'A szalon megadása kötelező.'});const raw=String(req.query.service_ids||'').split(',').map(x=>x.trim()).filter(Boolean);if(!raw.length)return res.json({requirements:[],missing_required:0});const {rows}=await db.query(`

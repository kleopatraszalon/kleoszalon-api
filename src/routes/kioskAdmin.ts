  ...
  if (menu?.id) {
    ...
    for (const r of rows) {
      if (!by.has(r.section_id)) {
        const sec = { id: r.section_id, title: r.title_hu, order: r.display_order, items: [] as any[] };
...
router.post("/menu/init", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const menuId = mRows[0].id as string;
    ...
    for (const t of types) {
      const { rows: sRows } = await pool.query(
        `INSERT INTO kiosk_menu_sections(menu_id, title_hu, display_order) VALUES ($1,$2,$3) RETURNING id`,
        [menuId, t.title, idx++]
      );
    ...
    const otherId = otherRows[0].id as string;
    ...
    for (const s of services) {
      ...
      await pool.query(
        `INSERT INTO kiosk_menu_items(section_id, service_id, display_order, enabled) VALUES ($1,$2,$3,true)
         ON CONFLICT(section_id, service_id) DO UPDATE SET enabled=true, display_order=EXCLUDED.display_order`,
        [secId, s.id, order++]
      );
    }
 ...
 */
router.put("/menu/:menuId/theme", async (req: Request, res: Response) => {
  const { menuId } = req.params;
  const theme = req.body?.theme ?? {};
  await pool.query(
    `UPDATE kiosk_menus SET theme=$2::jsonb, updated_at=now() WHERE id=$1`,
    [menuId, JSON.stringify(theme)]
  );
  res.json({ ok: true });
});
 ...
 */
router.put("/menu/:menuId/items", async (req: Request, res: Response) => {
  const { menuId } = req.params;
  const sections = (req.body?.sections ?? []) as any[];
  ...
  try {
    ...
    for (const sec of sections) {
      const sectionId = sec.sectionId as string;

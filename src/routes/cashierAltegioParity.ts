...
router.get('/altegio/accounts',async(req:AuthRequest,res,next)=>{try{const loc=locationFrom(req);const {rows}=await db.query(`SELECT a.*,a.opening_balance+COALESCE(SUM(CASE WHEN m.direction='income' THEN m.amount ELSE -m.amount END),0)::numeric current_balance FROM financial_accounts a LEFT JOIN financial_movements m ON m.account_id=a.id WHERE a.active=true AND ($1='' OR a.location_id::text=$1 OR a.location_id IS NULL) GROUP BY a.id ORDER BY a.sort_order,a.name`,[loc]);res.json(rows)}catch(e){next(e)}});
...
router.get('/workorders/:id/payments',async(req,res,next)=>{try{res.json((await db.query(`SELECT wp.*,a.name account_name,GREATEST(COALESCE(wp.refunded_amount,0),COALESCE((SELECT SUM(r.amount) FROM work_order_payment_refunds r WHERE r.payment_id=wp.id),0))::numeric effective_refunded_amount,COALESCE((SELECT json_agg(r ORDER BY r.created_at) FROM work_order_payment_refunds r WHERE r.payment_id=wp.id),'[]'::json)::json refunds FROM work_order_payments wp LEFT JOIN financial_accounts a ON a.id=wp.finance_account_id WHERE wp.work_order_id::text=$1 ORDER BY wp.paid_at,wp.id`,[req.params.id])).rows)}catch(e){next(e)}});

router.post('/payments/:id/refund',async(req:AuthRequest,res,next)=>{const client=await db.connect();try{
 const reason=String(req.body?.reason||'').trim(),requested=money(req.body?.amount),idempotencyKey=requireIdempotencyKey(req,'cashier-refund');if(reason.length<3)return res.status(400).json({message:'A visszatérítés indoka kötelező.'});await client.query('BEGIN');
 const payment=(await client.query(`SELECT wp.*,wo.location_id,wo.gross_total,wo.discount_amount,wo.tip_amount,GREATEST(COALESCE(wp.refunded_amount,0),COALESCE((SELECT SUM(r.amount) FROM work_order_payment_refunds r WHERE r.payment_id=wp.id),0))::numeric effective_refunded_amount FROM work_order_payments wp JOIN work_orders wo ON wo.id=wp.work_order_id WHERE wp.id=$1::uuid FOR UPDATE`,[req.params.id])).rows[0];if(!payment){await client.query('ROLLBACK');return res.status(404).json({message:'A fizetés nem található.'})}
 const existing=(await client.query(`SELECT * FROM work_order_payment_refunds WHERE payment_id=$1::uuid AND idempotency_key=$2 FOR UPDATE`,[payment.id,idempotencyKey])).rows[0];if(existing){await client.query('COMMIT');return res.json({refund:existing,idempotent:true})}
 const available=money(Number(payment.amount)-Number(payment.effective_refunded_amount||0)),amount=requested>0?Math.min(requested,available):available;if(!(amount>0)){await client.query('ROLLBACK');return res.status(409).json({message:'A fizetés már teljes egészében visszatérített.'})}
 ...
 if(!accountId){await client.query('ROLLBACK');return res.status(400).json({message:'A visszatérítéshez pénzügyi számla szükséges.'})}
 const movement=(await client.query(`INSERT INTO financial_movements(location_id,account_id,direction,amount,occurred_at,reference_type,reference_id,note,created_by,payment_method_code,document_type_code,payment_status,idempotency_key,posting_group_id) VALUES($1,$2::uuid,'expense',$3,now(),'cashier_refund',$4,$5,$6,$7,'refund','posted',$8,gen_random_uuid()) RETURNING *`,[payment.location_id,accountId,amount,String(payment.work_order_id),reason,actor(req),String(payment.payment_method_code||payment.payment_method),`${idempotencyKey}:ledger`])).rows[0];
 const refund=(await client.query(`INSERT INTO work_order_payment_refunds(payment_id,work_order_id,location_id,finance_account_id,cashier_shift_id,amount,reason,refund_method,created_by,financial_movement_id,idempotency_key,integrity_required) VALUES($1::uuid,$2,$3,$4::uuid,$5,$6,$7,$8,$9,$10::uuid,$11,true) RETURNING *`,[payment.id,String(payment.work_order_id),String(payment.location_id),accountId,shift?.id||payment.cashier_shift_id||null,amount,reason,String(payment.payment_method_code||payment.payment_method),actor(req),movement.id,idempotencyKey])).rows[0];
 if(payment.payment_method==='cash')await client.query(`INSERT INTO cash_register_movements(location_id,business_date,direction,amount,reason_code,note,created_by,transaction_type_code,reference_no,finance_account_id,cashier_shift_id,financial_movement_id,idempotency_key,integrity_required) VALUES($1,CURRENT_DATE,'out',$2,'refund',$3,$4,'refund',$5,$6::uuid,$7,$8::uuid,$9,true)`,[String(payment.location_id),amount,reason,actor(req),String(payment.id),accountId,shift.id,movement.id,`${idempotencyKey}:cash`]);
 const paid=money((await client.query(`SELECT COALESCE(SUM(wp.amount-GREATEST(COALESCE(wp.refunded_amount,0),COALESCE((SELECT SUM(r.amount) FROM work_order_payment_refunds r WHERE r.payment_id=wp.id),0))),0)::numeric paid FROM work_order_payments wp WHERE wp.work_order_id=$1`,[payment.work_order_id])).rows[0]?.paid),due=Math.max(0,money(Number(payment.gross_total||0)-Number(payment.discount_amount||0)+Number(payment.tip_amount||0))),refundCount=Number((await client.query(`SELECT count(*) n FROM work_order_payment_refunds WHERE work_order_id=$1`,[String(payment.work_order_id)])).rows[0]?.n||0),status=paid<=0&&refundCount>0?'refunded':paid<=0?'unpaid':paid+.009<due?'partial':'paid';
 await client.query(`UPDATE work_orders SET amount_paid=$2,payment_status=$3,fully_paid=($3='paid'),updated_at=now() WHERE id=$1`,[payment.work_order_id,paid,status]);await client.query(`INSERT INTO finance_integrity_events(event_type,location_key,subject_type,subject_id,actor,reason,evidence) VALUES('payment_refunded',$1,'work_order_payment_refund',$2,$3,$4,$5::jsonb)`,[String(payment.location_id),String(refund.id),actor(req),reason,JSON.stringify({payment_id:payment.id,movement_id:movement.id,amount})]);await client.query('COMMIT');res.status(201).json({refund,amount_paid:paid,payment_status:status,idempotent:false});
}catch(e){await client.query('ROLLBACK').catch(()=>undefined);next(e)}finally{client.release()}});
...
router.post('/manual-operation',async(req:AuthRequest,res,next)=>{const client=await db.connect();try{
 const locationId=locationFrom(req),accountId=String(req.body?.account_id||'').trim(),direction=String(req.body?.direction||''),amount=money(req.body?.amount),documentType=String(req.body?.document_type_code||'').trim()||'other',idempotencyKey=requireIdempotencyKey(req,'cashier-manual');if(!locationId||!accountId||!['income','expense'].includes(direction)||!(amount>0))return res.status(400).json({message:'Telephely, pénzügyi számla, irány és pozitív összeg szükséges.'});await client.query('BEGIN');
 ...
 const existing=(await client.query(`SELECT * FROM financial_movements WHERE location_id::text=$1 AND idempotency_key=$2 FOR UPDATE`,[String(account.location_id||locationId),idempotencyKey])).rows[0];if(existing){await client.query('COMMIT');return res.json({...existing,idempotent:true})}
 const movement=(await client.query(`INSERT INTO financial_movements(location_id,account_id,direction,amount,occurred_at,reference_type,reference_id,counterparty,note,created_by,partner_id,payment_method_code,document_type_code,employee_id,payment_status,idempotency_key,posting_group_id) VALUES($1,$2::uuid,$3,$4,COALESCE($5::timestamptz,now()),'cashier_manual',$6,$7,$8,$9,$10,$11,$12,$13,'posted',$14,gen_random_uuid()) RETURNING *`,[account.location_id||locationId,accountId,direction,amount,req.body?.occurred_at||null,req.body?.reference_no||null,req.body?.counterparty||null,req.body?.note||null,actor(req),req.body?.partner_id||null,req.body?.payment_method_code||null,documentType,req.body?.employee_id||null,idempotencyKey])).rows[0];
 if(account.account_type==='cash')await client.query(`INSERT INTO cash_register_movements(location_id,business_date,direction,amount,reason_code,note,created_by,transaction_type_code,reference_no,partner_id,employee_id,finance_account_id,cashier_shift_id,financial_movement_id,idempotency_key,integrity_required) VALUES($1,CURRENT_DATE,$2,$3,$4,$5,$6,$4,$7,$8,$9,$10::uuid,$11,$12::uuid,$13,true)`,[locationId,direction==='income'?'in':'out',amount,documentType,req.body?.note||null,actor(req),req.body?.reference_no||null,req.body?.partner_id||null,req.body?.employee_id||null,accountId,shift.id,movement.id,`${idempotencyKey}:cash`]);
...
router.post('/account-transfer',async(req:AuthRequest,res,next)=>{const client=await db.connect();try{
 const locationId=locationFrom(req),source=String(req.body?.source_account_id||'').trim(),destination=String(req.body?.destination_account_id||'').trim(),amount=money(req.body?.amount),idempotencyKey=requireIdempotencyKey(req,'cashier-transfer');if(!locationId||!source||!destination||source===destination||!(amount>0))return res.status(400).json({message:'Két külön pénzügyi számla és pozitív összeg szükséges.'});await client.query('BEGIN');const existing=(await client.query(`SELECT * FROM financial_transfers WHERE idempotency_key=$1 FOR UPDATE`,[idempotencyKey])).rows[0];if(existing){await client.query('COMMIT');return res.json({...existing,idempotent:true})}const accounts=(await client.query(`SELECT * FROM financial_accounts WHERE id=ANY($1::uuid[]) AND active=true FOR UPDATE`,[[source,destination]])).rows;if(accounts.length!==2){await client.query('ROLLBACK');return res.status(404).json({message:'Az egyik pénzügyi számla nem található.'})}const sourceAccount=accounts.find((a:any)=>String(a.id)===source),destAccount=accounts.find((a:any)=>String(a.id)===destination),shift=await currentShift(locationId);if((sourceAccount.account_type==='cash'||destAccount.account_type==='cash')&&!shift){await client.query('ROLLBACK');return res.status(409).json({message:'Készpénzt érintő átvezetéshez nyitott pénztári műszak szükséges.'})}
 const out=(await client.query(`INSERT INTO financial_movements(location_id,account_id,direction,amount,occurred_at,reference_type,reference_id,note,created_by,document_type_code,payment_status,posting_group_id,idempotency_key) VALUES($1,$2::uuid,'expense',$3,now(),'transfer',$4,$5,$6,'transfer','posted',gen_random_uuid(),$7) RETURNING id,posting_group_id`,[sourceAccount.location_id||locationId,source,amount,req.body?.reference_no||null,req.body?.note||null,actor(req),`${idempotencyKey}:out`])).rows[0];const inc=(await client.query(`INSERT INTO financial_movements(location_id,account_id,direction,amount,occurred_at,reference_type,reference_id,note,created_by,document_type_code,payment_status,posting_group_id,idempotency_key) VALUES($1,$2::uuid,'income',$3,now(),'transfer',$4,$5,$6,'transfer','posted',$7,$8) RETURNING id`,[destAccount.location_id||locationId,destination,amount,req.body?.reference_no||null,req.body?.note||null,actor(req),out.posting_group_id,`${idempotencyKey}:in`])).rows[0];
 const transfer=(await client.query(`INSERT INTO financial_transfers(location_id,source_account_id,destination_account_id,amount,transferred_at,note,created_by,source_movement_id,destination_movement_id,idempotency_key) VALUES($1::uuid,$2::uuid,$3::uuid,$4,now(),$5,$6,$7::uuid,$8::uuid,$9) RETURNING *`,[sourceAccount.location_id||destAccount.location_id||locationId,source,destination,amount,req.body?.note||null,actor(req),out.id,inc.id,idempotencyKey])).rows[0];
 if(shift&&sourceAccount.account_type==='cash')await client.query(`INSERT INTO cash_register_movements(location_id,business_date,direction,amount,reason_code,note,created_by,transaction_type_code,reference_no,finance_account_id,cashier_shift_id,financial_movement_id,idempotency_key,integrity_required) VALUES($1,CURRENT_DATE,'out',$2,'transfer',$3,$4,'transfer',$5,$6::uuid,$7,$8::uuid,$9,true)`,[locationId,amount,req.body?.note||'Pénztárközi átvezetés',actor(req),req.body?.reference_no||null,source,shift.id,out.id,`${idempotencyKey}:cash-out`]);if(shift&&destAccount.account_type==='cash')await client.query(`INSERT INTO cash_register_movements(location_id,business_date,direction,amount,reason_code,note,created_by,transaction_type_code,reference_no,finance_account_id,cashier_shift_id,financial_movement_id,idempotency_key,integrity_required) VALUES($1,CURRENT_DATE,'in',$2,'transfer',$3,$4,'transfer',$5,$6::uuid,$7,$8::uuid,$9,true)`,[locationId,amount,req.body?.note||'Pénztárközi átvezetés',actor(req),req.body?.reference_no||null,destination,shift.id,inc.id,`${idempotencyKey}:cash-in`]);
 await client.query('COMMIT');res.status(201).json({...transfer,idempotent:false});
}catch(e){await client.query('ROLLBACK').catch(()=>undefined);next(e)}finally{client.release()}});

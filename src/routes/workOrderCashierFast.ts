...
router.post('/workorders/:id/settle',async(req:AuthRequest,res,next)=>{
  // Ha valódi pont/wallet/kupon/bérlet beváltás történik, a teljes loyalty cashier kezelje.
  ...
  try{
    ...
    if(!hasOrders||!hasItems||!hasPayments)return res.status(503).json({message:'A munkalap pénztári alaptáblái még nem állnak rendelkezésre.',code:'WORKORDER_CASHIER_SCHEMA_MISSING'});
    const woCols=await columns('work_orders');
    const required=['payment_status','financial_closed_at'];
    const missing=required.filter(x=>!woCols.has(x));
    if(missing.length)return res.status(503).json({message:'A munkalap pénzügyi sémája hiányos.',code:'WORKORDER_CASHIER_COLUMNS_MISSING',missing});
    ...
    if(closeFinancially&&String(wo.status||'')!=='in_progress'){await c.query('ROLLBACK');return res.status(409).json({message:'Végleges pénzügyi zárás csak Folyamatban állapotú munkalapon végezhető.'})}
    ...
    for(const p of incoming){
      ...
      await c.query(`INSERT INTO work_order_payments(work_order_id,payment_method,amount,paid_at,note) VALUES($1::uuid,$2,$3,now(),$4)`,[req.params.id,method,amount,p?.note||null]);
    }
    ...
    const gross=money(grossQ.rows[0]?.gross),paid=money(paidQ.rows[0]?.paid),due=Math.max(0,money(gross-discount+tip));
    ...
    const add=(col:string,val:any,cast='')=>{if(!woCols.has(col))return;params.push(val);sets.push(`${col}=$${params.length}${cast}`)};
    add('gross_total',gross);add('discount_amount',discount);add('tip_amount',tip);add('amount_due',due);add('amount_paid',paid);add('payment_status',paymentStatus);add('fully_paid',paymentStatus==='paid');add('invoice_status',invoiceStatus);
    if(closeFinancially){if(woCols.has('financial_closed_at'))sets.push('financial_closed_at=COALESCE(financial_closed_at,now())');if(woCols.has('financial_closed_by')){params.push(req.user?.email||String(req.user?.id||''));sets.push(`financial_closed_by=COALESCE(financial_closed_by,$${params.length})`)}}
    ...
    return res.json({...updated,fast:true,loyalty_passthrough:false});
  }catch(e:any){

...
r.post('/submissions/:id/refresh',async(req:AuthRequest,res)=>{
 try{
  ...
  // A státuszt mindig ugyanabban a NAV környezetben kell visszakérdezni, ahová a
  // transactionId létrejött. Ha közben átállították a konfigurációt, inkább fail-closed:
  // másik környezet hitelesítő adataival nem próbálunk lekérdezni semmit.
  ...
  const processing=text.match(/<processingResult>([\s\S]*?)<\/processingResult>/)?.[1]||'';
  const status=processing.match(/<invoiceStatus>([^<]+)<\/invoiceStatus>/)?.[1]||'PROCESSING';
  const mapped=status==='DONE'?'done':status==='ABORTED'?'aborted':status==='WARN'?'warning':status==='ERROR'?'error':'processing';
  const code=processing.match(/<validationResultCode>([^<]+)<\/validationResultCode>/)?.[1]||null;
  const msg=processing.match(/<message>([^<]+)<\/message>/)?.[1]||null;
  await db.query(`UPDATE nav_invoice_submissions SET status=$2,response_xml=$3,nav_result=$4::jsonb,error_code=$5,error_message=$6,completed_at=CASE WHEN $2 IN ('done','warning','error','aborted') THEN now() ELSE completed_at END,updated_at=now() WHERE id=$1`,[s.id,mapped,text,JSON.stringify({invoiceStatus:status,validationResultCode:code,message:msg}),code,msg]);
  await db.query(`UPDATE finance_invoices SET nav_status=$2 WHERE id=$1`,[s.invoice_id,mapped]);
  res.json({ok:true,status:mapped,invoice_status:status,validation_code:code,message:msg,environment:submissionEnvironment})

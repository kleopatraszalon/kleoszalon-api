...
router.post("/v4/chain-book",async(req,res)=>{
  ...
  const customerNote=String(req.body?.note||"").trim().slice(0,1000);
  ...
  try{
    ...
    for(let i=0;i<resolved.length;i++){
      ...
      const appointmentNotes=[customerNote,`Booking 4.0 lánc: ${chain.id}`].filter(Boolean).join("\n");
      const ap=(await cx.query(`INSERT INTO appointments(employee_id,client_id,location_id,title,start_time,end_time,status,notes,booking_source,cancellation_token,confirmation_required,confirmed_at,updated_at) VALUES($1::uuid,$2::uuid,$3::uuid,$4,$5::timestamptz,$6::timestamptz,$7,$8,'online',$9::uuid,$10,$11,now()) RETURNING id`,[x.employee_id,client.id,locationId,x.service.name,x.start,x.end,status,appointmentNotes,token,Boolean(cfg.require_staff_confirmation),cfg.require_staff_confirmation?null:new Date().toISOString()])).rows[0];

...
router.post('/passes',async(req,res,next)=>{const client=await db.connect();try{await client.query('BEGIN');const type=await client.query(`SELECT * FROM loyalty_pass_types WHERE id=$1::uuid`,[req.body?.pass_type_id]);if(!type.rows[0])throw new Error('Bérlettípus nem található');const from=req.body?.valid_from||new Date().toISOString().slice(0,10);const r=await client.query(`INSERT INTO loyalty_passes(account_id,pass_type_id,valid_from,valid_until) VALUES($1::uuid,$2::uuid,$3::date,CASE WHEN $4::int IS NULL THEN NULL ELSE ($3::date + $4::int) END) RETURNING *`,[req.body?.account_id,req.body?.pass_type_id,from,type.rows[0].valid_days]);await client.query(`INSERT INTO loyalty_pass_balances(pass_id,service_id,original_quantity,remaining_quantity) SELECT $1,service_id,quantity,quantity FROM loyalty_pass_type_services WHERE pass_type_id=$2`,[r.rows[0].id,req.body?.pass_type_id]);await client.query('COMMIT');res.status(201).json(r.rows[0])}catch(err){await client.query('ROLLBACK');next(err)}finally{client.release()}});

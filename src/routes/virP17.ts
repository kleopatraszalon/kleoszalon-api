...
const OPERATION_TYPES = new Set(["capacity_review","staffing_review","inventory_review","revenue_review","customer_retention_review","manual_task"]);
...
function riskFor(type:string):"low"|"medium"|"high"{if(type==="manual_task")return "low";if(type==="staffing_review"||type==="revenue_review"||type==="customer_retention_review")return "high";return "medium";}
...
async function transition(req:AuthRequest,res:Response,options:TransitionOptions){
  ...
  try{
    ...
    await client.query("COMMIT");return res.json({ok:true,operation:updated,external_side_effects:false});
...
router.post("/operations",async(req:AuthRequest,res:Response)=>{
  const tenant=tenantId(req,res);if(!tenant)return;
  try{const operationType=String(req.body?.operation_type||"").trim().toLowerCase();if(!OPERATION_TYPES.has(operationType))return res.status(400).json({ok:false,error:"unsupported_operation_type",allowed:[...OPERATION_TYPES]});const location=await validateLocation(req.body?.locationId,tenant,res);if(location===undefined)return;const title=String(req.body?.title||operationType).trim().slice(0,180);if(!title)return res.status(400).json({ok:false,error:"title_required"});const idempotencyKey=String(req.body?.idempotency_key||"").trim().slice(0,160)||null;if(idempotencyKey){const existing=(await pool.query(`SELECT * FROM vir_p17_operations WHERE tenant_id=$1::bigint AND idempotency_key=$2 LIMIT 1`,[tenant,idempotencyKey])).rows[0];if(existing)return res.json({ok:true,idempotent_replay:true,operation:existing});}const preview=cleanObject(req.body?.preview_payload),actor=actorId(req),risk=riskFor(operationType),sourceLayer=String(req.body?.source_layer||"manual").slice(0,40),sourceRef=String(req.body?.source_ref||"").slice(0,180)||null,client=await pool.connect();try{await client.query("BEGIN");const operation=(await client.query(`INSERT INTO vir_p17_operations(tenant_id,location_id,operation_type,title,status,execution_mode,approval_required,risk_level,source_layer,source_ref,idempotency_key,preview_payload,created_by) VALUES($1::bigint,$2::uuid,$3,$4,'pending_approval','controlled_manual',true,$5,$6,$7,$8,$9::jsonb,$10) RETURNING *`,[tenant,location,operationType,title,risk,sourceLayer,sourceRef,idempotencyKey,JSON.stringify(preview),actor])).rows[0];await appendEvent(client,tenant,String(operation.id),"created",actor,{operation_type:operationType,risk_level:risk,source_layer:sourceLayer,source_ref:sourceRef});await client.query("COMMIT");return res.status(201).json({ok:true,operation});}catch(error){await client.query("ROLLBACK").catch(()=>undefined);throw error;}finally{client.release();}}
...
router.post("/operations/:id/reject",async(req:AuthRequest,res:Response)=>{
  const tenant=tenantId(req,res);if(!tenant)return;const id=String(req.params.id||"");if(!UUID.test(id))return res.status(400).json({ok:false,error:"invalid_operation_id"});
  const row=(await pool.query(`UPDATE vir_p17_operations SET status='rejected',updated_at=now() WHERE id=$1::uuid AND tenant_id=$2::bigint AND status='pending_approval' RETURNING *`,[id,tenant])).rows[0];
  if(!row)return res.status(404).json({ok:false,error:"operation_not_found_or_not_pending"});return res.json({ok:true,operation:row,external_side_effects:false});
});

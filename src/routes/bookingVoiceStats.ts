  ...
  try {
    ...
    const [eventSummary, bookingSummary, intents, locations, eventDaily, bookingDaily, recent, services] = await Promise.all([
      db.query(`SELECT
          ...
          count(*) FILTER(WHERE recognized)::int recognized,
          count(*) FILTER(WHERE ai_used)::int ai_used,
          count(*) FILTER(WHERE intent='book')::int book_intents,
          count(*) FILTER(WHERE intent='book' AND recognized)::int recognized_book_intents,
          count(*) FILTER(WHERE intent='waitlist')::int waitlist_intents,
          count(*) FILTER(WHERE intent='cancel')::int cancel_intents,
          COALESCE(avg(transcript_length),0)::numeric avg_transcript_length
        FROM booking_voice_events
        WHERE created_at >= $1::timestamptz AND created_at < $2::timestamptz
          AND ($3::uuid IS NULL OR location_id=$3::uuid)`, params),
      db.query(`SELECT
          count(*) FILTER(WHERE a.booking_source='online_voice')::int voice_bookings,
          count(*) FILTER(WHERE a.booking_source='online')::int online_bookings
        FROM appointments a
        WHERE ${appointmentCreated} >= $1::timestamptz AND ${appointmentCreated} < $2::timestamptz
          AND ($3::uuid IS NULL OR a.location_id=$3::uuid)
          AND a.status NOT IN ('cancelled','canceled')`, params),
      ...
      db.query(`SELECT to_char(created_at AT TIME ZONE 'Europe/Budapest','YYYY-MM-DD') date,
          count(*)::int voice_requests,
          count(*) FILTER(WHERE recognized)::int recognized,
          count(*) FILTER(WHERE ai_used)::int ai_used
        FROM booking_voice_events
    ...
    let waitlist = { voice_waitlist: 0, online_waitlist: 0 };
    try {
      const r = await db.query(`SELECT
        ...
        WHERE created_at >= $1::timestamptz AND created_at < $2::timestamptz
          AND ($3::uuid IS NULL OR location_id=$3::uuid)`, params);
      waitlist = { voice_waitlist: count(r.rows[0]?.voice_waitlist), online_waitlist: count(r.rows[0]?.online_waitlist) };
    } catch (error: any) {
      if (!['42P01','42703'].includes(String(error?.code || ''))) throw error;
    }
    ...
    for (let i = days - 1; i >= 0; i -= 1) {
      ...
      daily.set(key,{date:key,voice_requests:0,recognized:0,ai_used:0,voice_bookings:0,online_bookings:0});
    }
    eventDaily.rows.forEach((r:any)=>daily.set(String(r.date),{...(daily.get(String(r.date))||{date:String(r.date)}),voice_requests:count(r.voice_requests),recognized:count(r.recognized),ai_used:count(r.ai_used)}));
    ...
    res.json({
      ...
      summary:{
        ...
        conversion_rate:pct(voiceBookings,voiceBookIntents),
      ...
      locations:locations.rows.map((r:any)=>({...r,voice_requests:count(r.voice_requests),recognized:count(r.recognized),voice_bookings:count(r.voice_bookings),online_bookings:count(r.online_bookings),recognition_rate:pct(count(r.recognized),count(r.voice_requests)),conversion_rate:pct(count(r.voice_bookings),count(r.recognized))})),

...
function roles(req:AuthRequest){
 ...
 const arr=Array.isArray(raw)?raw:String(raw||"").replace(/[\[\]"]/g,"").split(",");
...
router.get("/",async(req:AuthRequest,res,next)=>{
 ...
 try{
  ...
  const add=(x:Result)=>results.push(x);
  ...
  if(tableMap.get("menus")){const c=await count(`SELECT COUNT(*) FROM menus WHERE COALESCE(is_active,true)`);add({key:"menu.active",group:"Menü és jogosultság",label:"Aktív menüpontok",status:c>0?"ok":"error",count:c,message:c?`${c} aktív menüpont.`:"Nincs aktív menüpont."});}
  ...
  if(tableMap.get("accounting_journal_entries")&&tableMap.get("accounting_journal_lines")){
   const c=await count(`SELECT COUNT(*) FROM (SELECT je.id FROM accounting_journal_entries je JOIN accounting_journal_lines jl ON jl.journal_entry_id=je.id GROUP BY je.id HAVING ABS(COALESCE(SUM(jl.debit),0)-COALESCE(SUM(jl.credit),0))>0.01) q`);
   add({key:"ledger.balance",group:"Könyvelés",label:"Tartozik / Követel egyensúly",status:c?"error":"ok",count:c,message:c?`${c} kiegyensúlyozatlan naplótétel.`:"A főkönyvi tételek egyensúlyban vannak."});
  }

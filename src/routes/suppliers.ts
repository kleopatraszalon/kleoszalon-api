  ...
  try {
    const { rows } = await db.query(
      `UPDATE suppliers SET
         name=COALESCE(NULLIF($2,''),name), tax_number=$3, email=$4, phone=$5, contact_name=$6,
         address=$7, website=$8, payment_terms_days=$9, default_lead_time_days=$10,
         active=$11, note=$12, updated_at=now()
  ...
  try {
    const { rows } = await db.query(
      `SELECT pst.*, p.name AS product_name, p.internal_code, p.brand,
              COALESCE(hist.last_price,pst.unit_price)::numeric AS last_purchase_price,
              COALESCE(hist.avg_price,pst.unit_price)::numeric AS average_purchase_price,
              hist.last_received_at
       FROM product_supplier_terms pst
       JOIN products p ON p.id=pst.product_id
       LEFT JOIN LATERAL (
         SELECT poi.actual_unit_cost AS last_price,
                avg(COALESCE(poi.actual_unit_cost,poi.unit_cost))::numeric AS avg_price,
                max(po.received_at) AS last_received_at
         FROM purchase_order_items poi
         JOIN purchase_orders po ON po.id=poi.purchase_order_id
         WHERE poi.product_id=pst.product_id AND po.supplier_id=pst.supplier_id
       ) hist ON true
       WHERE pst.supplier_id=$1
       ORDER BY pst.preferred DESC, p.name`, [req.params.id]
    );
    res.json(rows);
  } catch (err) { next(err); }
});
...
router.put("/:supplierId/products/:productId", async (req, res, next) => {
  ...
  try {
    ...
    if (preferred) {
      await client.query(`UPDATE product_supplier_terms SET preferred=false, updated_at=now() WHERE product_id=$1`, [req.params.productId]);
    }
    const { rows } = await client.query(
      `INSERT INTO product_supplier_terms
       ...
       ON CONFLICT(product_id,supplier_id) DO UPDATE SET
         supplier_product_code=EXCLUDED.supplier_product_code, unit_price=EXCLUDED.unit_price,
         minimum_order_quantity=EXCLUDED.minimum_order_quantity, lead_time_days=EXCLUDED.lead_time_days,
         preferred=EXCLUDED.preferred, active=EXCLUDED.active, note=EXCLUDED.note, updated_at=now()
       RETURNING *`,
  ...
  try {
    ...
    const { rows } = await db.query(
      `WITH need AS (
         ...
         SELECT n.*,p.name product_name,p.internal_code,p.brand,
                pst.supplier_id,s.name supplier_name,pst.unit_price supplier_price,
       ...
       SELECT *,
              CASE WHEN minimum_order_quantity IS NULL THEN base_needed
                   ELSE GREATEST(base_needed,minimum_order_quantity) END::numeric AS suggested_quantity,
              (CASE WHEN minimum_order_quantity IS NULL THEN base_needed
                    ELSE GREATEST(base_needed,minimum_order_quantity) END * COALESCE(supplier_price,unit_cost,0))::numeric AS expected_cost
       FROM ranked WHERE rn=1 OR rn IS NULL
       ORDER BY supplier_name NULLS LAST, product_name`, [locationId]
    );

...
router.get('/dashboard',async(req:AuthRequest,res,next)=>{try{
 ...
 const p=[loc,from,to];
 const accounts=await db.query(`SELECT a.*,a.opening_balance+COALESCE(SUM(CASE WHEN m.cancelled_at IS NULL AND m.direction='income' THEN m.amount WHEN m.cancelled_at IS NULL THEN -m.amount ELSE 0 END),0)::numeric current_balance,
  COALESCE(SUM(CASE WHEN m.cancelled_at IS NULL AND m.direction='income' AND m.occurred_at>=$2::date AND m.occurred_at<($3::date+1) THEN m.amount ELSE 0 END),0)::numeric period_income,
  COALESCE(SUM(CASE WHEN m.cancelled_at IS NULL AND m.direction='expense' AND m.occurred_at>=$2::date AND m.occurred_at<($3::date+1) THEN m.amount ELSE 0 END),0)::numeric period_expense
 ...
 const timeline=await db.query(`SELECT date_trunc('month',m.occurred_at)::date month,
  COALESCE(SUM(m.amount) FILTER(WHERE m.direction='income' AND m.cancelled_at IS NULL),0)::numeric income,
  COALESCE(SUM(m.amount) FILTER(WHERE m.direction='expense' AND m.cancelled_at IS NULL),0)::numeric expense
 ...
 const composition=await db.query(`SELECT COALESCE(c.name,m.document_type_code,m.reference_type,'Egyéb') label,m.direction,COALESCE(SUM(m.amount),0)::numeric amount FROM financial_movements m LEFT JOIN financial_categories c ON c.id=m.category_id WHERE ($1::text IS NULL OR m.location_id::text=$1 OR m.location_id IS NULL) AND m.cancelled_at IS NULL AND m.occurred_at>=$2::date AND m.occurred_at<($3::date+1) GROUP BY 1,2 ORDER BY amount DESC LIMIT 30`,p);
...
router.get('/partners',async(req:AuthRequest,res,next)=>{try{const loc=requestedLocation(req),q=String(req.query.q||'').trim();const {rows}=await db.query(`SELECT p.*,COALESCE(SUM(CASE WHEN m.cancelled_at IS NULL AND m.direction='income' THEN m.amount WHEN m.cancelled_at IS NULL THEN -m.amount ELSE 0 END),0)::numeric balance FROM finance_partners p LEFT JOIN financial_movements m ON m.partner_id=p.id WHERE ($1::text IS NULL OR p.location_id=$1 OR p.location_id IS NULL) AND ($2='' OR p.name ILIKE '%'||$2||'%' OR COALESCE(p.tax_number,'') ILIKE '%'||$2||'%') GROUP BY p.id ORDER BY p.active DESC,p.name`,[loc,q]);res.json(rows)}catch(e){next(e)}});
...
router.post('/operations/:id/cancel',async(req:AuthRequest,res,next)=>{try{const reason=String(req.body?.reason||'').trim();if(!reason)return res.status(400).json({message:'A visszavonás indoka kötelező.'});const {rows}=await db.query(`UPDATE financial_movements SET cancelled_at=now(),cancelled_by=$2,cancellation_reason=$3 WHERE id=$1::uuid AND cancelled_at IS NULL RETURNING *`,[req.params.id,actor(req),reason]);if(!rows[0])return res.status(409).json({message:'A tranzakció már vissza van vonva vagy nem található.'});res.json(rows[0])}catch(e){next(e)}});

router.get('/reports/summary',async(req:AuthRequest,res,next)=>{try{const loc=requestedLocation(req),from=String(req.query.from||new Date(new Date().getFullYear(),0,1).toISOString().slice(0,10)),to=String(req.query.to||new Date().toISOString().slice(0,10));const {rows}=await db.query(`SELECT m.direction,COALESCE(c.name,m.document_type_code,m.reference_type,'Egyéb') category,COALESCE(SUM(m.amount),0)::numeric amount FROM financial_movements m LEFT JOIN financial_categories c ON c.id=m.category_id WHERE ($1::text IS NULL OR m.location_id::text=$1 OR m.location_id IS NULL) AND m.cancelled_at IS NULL AND m.occurred_at>=$2::date AND m.occurred_at<($3::date+1) GROUP BY 1,2 ORDER BY 1,amount DESC`,[loc,from,to]);const totals=rows.reduce((a:any,r:any)=>{a[r.direction]=(a[r.direction]||0)+num(r.amount);return a},{income:0,expense:0});res.json({from,to,rows,totals,profit:money(totals.income-totals.expense)})}catch(e){next(e)}});
router.get('/reports/pnl',async(req:AuthRequest,res,next)=>{try{const loc=requestedLocation(req),year=Math.max(2020,Math.min(2100,Number(req.query.year||new Date().getFullYear())));const {rows}=await db.query(`SELECT extract(month from m.occurred_at)::int month,COALESCE(SUM(m.amount) FILTER(WHERE m.direction='income'),0)::numeric income,COALESCE(SUM(m.amount) FILTER(WHERE m.direction='expense'),0)::numeric expense FROM financial_movements m WHERE ($1::text IS NULL OR m.location_id::text=$1 OR m.location_id IS NULL) AND m.cancelled_at IS NULL AND extract(year from m.occurred_at)=$2 GROUP BY 1 ORDER BY 1`,[loc,year]);res.json({year,rows:rows.map((r:any)=>({...r,profit:money(num(r.income)-num(r.expense))}))})}catch(e){next(e)}});
router.get('/reports/partner-balances',async(req:AuthRequest,res,next)=>{try{const loc=requestedLocation(req);const {rows}=await db.query(`SELECT p.id,p.name,p.partner_type,p.tax_number,COALESCE(SUM(CASE WHEN m.cancelled_at IS NULL AND m.direction='income' THEN m.amount WHEN m.cancelled_at IS NULL THEN -m.amount ELSE 0 END),0)::numeric balance FROM finance_partners p LEFT JOIN financial_movements m ON m.partner_id=p.id WHERE ($1::text IS NULL OR p.location_id=$1 OR p.location_id IS NULL) GROUP BY p.id ORDER BY p.name`,[loc]);res.json(rows)}catch(e){next(e)}});

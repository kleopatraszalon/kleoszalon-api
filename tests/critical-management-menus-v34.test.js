...
test('AI Exception CAPA workqueue Major Incident and Resilience executive menus self-heal analytics parent with management permissions',()=>{
  ...
  for(const marker of [
    ...
    'analytics.capa_workqueue','CAPA vezetői munkasor','/finance/exception-command-center/capa/workqueue',
    ...
    "m.code IN('analytics','analytics.executive_ai','analytics.exception_center','analytics.exception_intelligence','analytics.exception_capa','analytics.capa_workqueue','analytics.major_incident','analytics.resilience_recovery')",
    'clearShortCache("menu:")',
  ]) assert.ok(src.includes(marker),marker);
  assert.match(src,/VALUES\('admin'\),\('manager'\)/);
});

...
test('AI Exception CAPA improvement queue Major Incident and Resilience executive menus self-heal analytics parent with management permissions',()=>{
  ...
  for(const marker of [
    ...
    'analytics.exception_capa_queue','CAPA fejlesztési javaslatok','/finance/exception-command-center/capa/improvement-recommendations',
    ...
    "m.code IN('analytics','analytics.executive_ai','analytics.exception_center','analytics.exception_intelligence','analytics.exception_capa','analytics.exception_capa_queue','analytics.major_incident','analytics.resilience_recovery')",
    'clearShortCache("menu:")',
  ]) assert.ok(src.includes(marker),marker);
  assert.match(src,/VALUES\('admin'\),\('manager'\)/);
});

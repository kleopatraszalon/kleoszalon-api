...
test('Viber and Meta callbacks are authenticated and persist channel identities',()=>{for(const x of ['X-Viber-Content-Signature','X-Hub-Signature-256','COMMUNICATION_WEBHOOK_SECRET','vir_client_channel_identities','subscriber','messenger','external_id'])assert.ok(hooks.toLowerCase().includes(x.toLowerCase()));});

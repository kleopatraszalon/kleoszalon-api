...
test('improvement workflow is tenant scoped and approval is fail closed',()=>{const p=read('src/routes/managementImprovement.ts');for(const marker of ['resolveTenantIdentity','locationBelongsToTenant','tenant_id=$2::bigint','request-approval','before_value IS NOT NULL AND after_value IS NOT NULL','project.approval_state !== "pending"','Nincs függő jóváhagyási kérelem.','A projekt csak jóváhagyás után zárható le.','CAPA intézkedés eredményességét igazolni kell.'])expect(p).toContain(marker)});

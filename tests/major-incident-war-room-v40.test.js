...
test('War Room action completion evidence is protected by application and database guards',()=>{const src=read('src/services/majorIncidentWarRoom.ts'),hard=read('src/services/majorIncidentWarRoomHardening.ts');for(const marker of ['completion_evidence','Akció lezárásához konkrét végrehajtási bizonyíték'])assert.ok(src.includes(marker),marker);for(const marker of ['kleo_major_incident_action_guard','trg_kleo_major_incident_action_guard',"NEW.status='done'","completion_evidence->>'description'","ERRCODE='23514'"])assert.ok(hard.includes(marker),marker)});

test('War Room heartbeat watchdog detects missing commander stale updates and overdue critical actions',()=>{const src=read('src/services/majorIncidentWarRoomWatchdog.ts');for(const marker of ['commander_missing','war_room_update_stale','critical_action_overdue','system-war-room-watchdog','recentlyAlerted',"cron.schedule('*/5 * * * *'",'Europe/Budapest'])assert.ok(src.includes(marker),marker)});
...
test('Major Incident API exposes summary detector watchdog detail command lifecycle actions and War Room updates',()=>{const route=read('src/routes/exceptionCommandCenter.ts');for(const marker of ['/intelligence/major-incidents/summary','/intelligence/major-incidents','/intelligence/major-incidents/sync','/intelligence/major-incidents/watchdog','/intelligence/major-incidents/:id','/actions/:actionId','/updates','startMajorIncidentWarRoomScheduler','startMajorIncidentWarRoomWatchdog','ensureMajorIncidentHardeningSchema','major_incident_governance_conflict'])assert.ok(route.includes(marker),marker)});

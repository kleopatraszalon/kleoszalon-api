...
test('SEV1 SEV2 recovery automatically opens change freeze and three-minute sync',()=>{const src=read('src/services/resilienceRecoveryControl.ts');for(const marker of ['resilience_change_freezes','SEV1','sev1','sev2','syncResilienceRecoveryControl',"cron.schedule('*/3 * * * *'",'Europe/Budapest'])assert.ok(src.includes(marker),marker)});
...
test('ALL CLEAR is fail closed at application and database layers',()=>{const src=read('src/services/resilienceRecoveryControl.ts'),hard=read('src/services/resilienceRecoveryHardening.ts');for(const marker of ['declareRecoveryAllClear','mandatoryOpen','unverified','openActions','ALL CLEAR','change-freeze feloldva'])assert.ok(src.includes(marker),marker);for(const marker of ['kleo_resilience_all_clear_guard',"incident_status NOT IN ('monitoring','resolved')",'mandatory recovery work remains','trg_kleo_resilience_freeze_reactivation_guard','nem aktiválható újra'])assert.ok(hard.includes(marker),marker)});

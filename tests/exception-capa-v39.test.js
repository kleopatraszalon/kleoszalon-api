...
test('CAPA API exposes summary list sync detail controlled update and location scope',()=>{const route=read('src/routes/exceptionCommandCenter.ts');for(const marker of ['/intelligence/capa/summary','/intelligence/capa','/intelligence/capa/sync','/intelligence/capa/:id','startExceptionCapaScheduler','ensureExceptionCapaHardeningSchema','capaVisible','!x.location_id||String(x.location_id)===location'])assert.ok(route.includes(marker),marker)});

...
test('CAPA database trigger prevents bypassing approval and verification evidence',()=>{const src=read('src/services/exceptionCapaHardening.ts');for(const marker of ['kleo_exception_capa_state_guard','trg_kleo_exception_capa_state_guard',"NEW.status IN ('approved','in_progress','verification','verified')",'approved_by','approved_at',"NEW.status='verified'",'verified_by','verified_at',"verification_evidence->>'description'", "ERRCODE='23514'"])assert.ok(src.includes(marker),marker)});

test('CAPA API exposes summary list sync detail controlled update location scope and governed conflicts',()=>{const route=read('src/routes/exceptionCommandCenter.ts');for(const marker of ['/intelligence/capa/summary','/intelligence/capa','/intelligence/capa/sync','/intelligence/capa/:id','startExceptionCapaScheduler','ensureExceptionCapaHardeningSchema','capaVisible','!x.location_id||String(x.location_id)===location','sendGovernanceError','capa_governance_conflict','23514'])assert.ok(route.includes(marker),marker)});

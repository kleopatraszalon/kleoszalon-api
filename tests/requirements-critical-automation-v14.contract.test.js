...
test('KLEO-NFR-SEC-003-AC-01: protected tokens are verified with the server-side signing key before authorization', () => {
  ...
  assert.match(middleware, /jwt\.verify\(credential\.token, JWT_SECRET\)/);
...
test('KLEO-NFR-SEC-003-AC-02: expired or modified tokens fail closed with HTTP 401 and no business handler execution', () => {
  ...
  const verifyPos = middleware.indexOf('jwt.verify(credential.token, JWT_SECRET)');
  const authorizePos = middleware.indexOf('enforceKnownModuleAccess(req, res)', verifyPos);
  const nextPos = middleware.indexOf('return next();', authorizePos);
  assert.ok(verifyPos >= 0 && authorizePos > verifyPos && nextPos > authorizePos, 'authorization and handler continuation must happen only after successful token verification');
});

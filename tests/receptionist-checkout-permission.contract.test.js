...
test('cashier settlement remains location-scoped and finance-feature protected',()=>{
  assert.match(tx,/router\.use\("\/cashier",workOrderFinanceScope,ensureFinanceReady,guardOpenCashierShift,requireFeature\("finance"\),requireMenuPermissionByMethod\("finance\.checkout"\),workOrderCashierFastRouter\)/);
});
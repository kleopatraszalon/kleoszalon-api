...
test('49 Channel Optimizer is consent and response aware',()=>{assert.ok(route.includes("model:'consent_cost_response_channel_optimizer_v1'"));assert.ok(route.includes('transactional_consent'));assert.ok(route.includes('marketing_consent'));assert.ok(route.includes('automatic_send:false'));});
test('50 Revenue Attribution discloses proxy and no causality claim',()=>{assert.ok(route.includes("model:'last_touch_booking_value_attribution_v1'"));assert.ok(route.includes('paid_revenue_supported:false'));assert.ok(route.includes('causality_claim:false'));assert.ok(route.includes('attribution_window_days:14'));});

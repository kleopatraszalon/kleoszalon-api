...
const db_1 = __importDefault(require("../db")); // ugyanaz, mint a többi route-ban!
...
router.patch("/:id", async (req, res) => {
    try {
        ...
        if (b.name !== undefined)
            add("name", b.name);
        if (b.internal_code !== undefined)
            add("internal_code", b.internal_code);
        if (b.barcode !== undefined)
            add("barcode", b.barcode);
        if (b.brand !== undefined)
            add("brand", b.brand);
        if (b.line_name !== undefined)
            add("line_name", b.line_name);
        ...
        if (b.vat_rate !== undefined)
            add("vat_rate", b.vat_rate);
        if (b.size_label !== undefined)
            add("size_label", b.size_label);
        if (b.color_text !== undefined)
            add("color_text", b.color_text);
        if (b.target_gender !== undefined)
            add("target_gender", b.target_gender);
        if (b.is_active !== undefined)
            add("is_active", b.is_active);
        if (b.is_service_material !== undefined)
            add("is_service_material", b.is_service_material);
        if (b.is_retail !== undefined)
            add("is_retail", b.is_retail);
        if (b.is_cleaning !== undefined)
            add("is_cleaning", b.is_cleaning);
...
router.post("/bulk-import", async (req, res) => {
    try {
        ...
        const { rows: groupRows } = await db_1.default.query("SELECT id, name, code FROM product_groups");
        const { rows: catRows } = await db_1.default.query("SELECT id, name, product_group_id FROM product_categories");
        let created = 0;
        const errors = [];
        for (let i = 0; i < items.length; i++) {
            ...
            if (item.product_group_name) {
                ...
                const g = groupRows.find((gr) => String(gr.name).toLowerCase() === gname ||
            ...
            if (item.product_category_name) {
                ...
                const match = catRows.find((cr) => String(cr.name).toLowerCase() === cname);

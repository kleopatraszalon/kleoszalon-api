...
router.get("/workorders/:id", async (req, res, next) => {
    try {
        const { id } = req.params;
        const header = await db_1.default.query(`SELECT * FROM v_work_order_details WHERE id = $1`, [id]);
        if (!header.rows[0]) {
            return res.status(404).json({ message: "A munkalap nem található" });
        }
        const items = await db_1.default.query(`SELECT id, item_type, service_id, product_id, item_name, quantity, unit_price, discount_amount, line_total, duration_minutes
       FROM work_order_items
       WHERE work_order_id = $1
       ORDER BY created_at`, [id]);
        const payments = await db_1.default.query(`SELECT id, payment_method, amount, paid_at, note
...
router.post("/workorders", async (req, res, next) => {
    ...
    try {
        const { title, notes, status, employee_id, client_name, client_phone, client_email, location_id, appointment_id, fully_paid, note_for_another_visitor, services, products, created_by, } = req.body;

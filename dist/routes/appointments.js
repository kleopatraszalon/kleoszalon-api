...
const pool = new pg_1.Pool(); // relies on PG* env vars
const createSchema = zod_1.z.object({
    location_id: zod_1.z.string().uuid(),
    employee_id: zod_1.z.string().uuid(),
    client_id: zod_1.z.string().uuid().optional().nullable(),
    service_id: zod_1.z.string().uuid(),
    title: zod_1.z.string().optional(),
    start_time: zod_1.z.string(), // 'YYYY-MM-DD HH:mm'
    end_time: zod_1.z.string(),
    price: zod_1.z.number().optional(),
    notes: zod_1.z.string().optional()
});
r.get("/", async (req, res) => {
    const { location_id, from, to, employee_id } = req.query;
    const q = `
...
r.get("/free-slots", async (req, res) => {
    const { location_id, employee_id, day, slot_minutes } = req.query;

...
router.get("/", async (req, res) => {
    const result = await db_1.default.query("SELECT * FROM roles ORDER BY level DESC");
...
router.post("/", async (req, res) => {
    ...
    try {
        const result = await db_1.default.query("INSERT INTO roles (name, description, level) VALUES ($1, $2, $3) RETURNING *", [name, description || "", level || 1]);
...
router.get("/:roleId/permissions", async (req, res) => {
    ...
    const result = await db_1.default.query(`SELECT rp.id, m.name AS menu, rp.can_view, rp.can_edit, rp.can_delete
...
router.put("/:roleId/permissions", async (req, res) => {
    ...
    try {
        for (const u of updates) {
            await db_1.default.query(`UPDATE role_permissions

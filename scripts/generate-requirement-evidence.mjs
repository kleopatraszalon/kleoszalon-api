...
const evidence=(automation.entries||[]).map(entry=>({requirement_id:entry.criterion_id.replace(/-AC-\d{2}$/,''),acceptance_criteria_id:entry.criterion_id,test_case_id:`TC-${entry.criterion_id}`,execution_type:entry.execution_type,result:'passed',build_ref:buildRef,environment,executed_by:actor,executed_at:generatedAt,evidence_ref:runUrl||`ci:${buildRef}`,evidence_payload:{test_ref:entry.test_ref,github_run_id:runId,repository:process.env.GITHUB_REPOSITORY||null}}));
...
const dir=path.join(root,'artifacts');fs.mkdirSync(dir,{recursive:true});const out=path.join(dir,'requirements-evidence.json');fs.writeFileSync(out,JSON.stringify(payload,null,2)+'\n','utf8');console.log(`Requirement evidence: ${evidence.length} passed automation records -> ${path.relative(root,out)}`);
